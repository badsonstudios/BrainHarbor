...
| **In progress** | **WI-538 (Pediatric brain tumor, deepened) — implementation COMPLETE, awaiting commit and PRs**, branch `feature/wi-538-pediatric-brain-tumor` (from develop at `9fa7570`). **The audience is a PARENT, not the patient** — the first page in the corpus whose reader is not the person with the tumor, so §12.11 governs alongside §12.3. **All gates green:** grade **5.6** against a hard 6.0, suite **2155/2155** (2117 before), ContentCheck **273/0**, **81 break-mutations caught on LF and CRLF**, **8 render guards proved by rebuild**, **five rendered reads**, **4 `/review` rounds** (2 blockers, then 1, then 1, then none), **0 corpus-restatement collisions** across 3,031 shingles. **Shipped:** the 47-line stub becomes a full cross-cutting page; new glossary entry `embryonal-tumor`; `/tumors/medulloblastoma` gained the suppression marker and LOST an unsourced "it is treatable" it had inherited from the same source with the same gap; `[SPINAL-CORD]` and `[POSTERIOR-FOSSA-SYNDROME]` both included and registered in exactly-asserted lists. **Docs done:** §12.8 entry pasted (513 lines, before §12.9), backlog entry replaced with the done-entry plus `/pm` carry-forwards. **WI-566 filed** for a test-helper defect live on four other pages. **REMAINING: privacy re-scan on the FINAL diff** (it must include the 513 new §12.8 lines) **→ commit → PR into `develop` → release PR into `main` → deploy smoke**, with a pre-deploy byte baseline and tooltip-free fragment probes per WI-537. **The item's through-line: my own verification produced FIVE false negatives by four distinct mechanisms** — hard wrapping twice, a comma where whitespace was assumed, and searching for only PART of a sentence being changed — **and the break harness left two survivors, both caused by front-matter comments this item wrote itself**, in which a first-occurrence mutation bit the comment instead of the prose. Three tools now exist so none of it is re-derived: `privacy-scan.py`, `shingles.py`, `anchor-audit.py`. |
