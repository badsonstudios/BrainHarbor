...
| **In progress** | **WI-547 (Hemangioblastoma) — WRITTEN AND FULLY PROVEN, NOT YET COMMITTED** (2026-09-22), branch `feature/wi-547-hemangioblastoma` (from develop @ `d921b7f`). WI-546 is SHIPPED AND LIVE — do not reopen it. **This was the LAST unwritten taxonomy type**, so `/tumors` no longer says "We are still writing this one" anywhere. **Gate 1 ruling: the assertion that depended on it was REWRITTEN, not deleted** — it now supplies its own unwritten type through the existing `Content:TaxonomyFile` setting and scopes its checks to that row, and a new companion asserts the complement on the real index. **Proof: suite 2,457/2,457; ContentCheck 277/0 at grade 4.8; 94 break mutations red on LF AND CRLF (188/188) after one first-run survivor that was a WEAK GUARD, rewritten rather than the mutation weakened; 12 handproof cases across all 7 render guards PLUS both halves of the Gate 1 ruling; both restatement probes 0 collisions with no allowlist; two rendered reads; FIVE `/review` rounds (3 blockers, 1, 2, none, none).** Rounds 2 and 3 each found their blocker inside the previous round's fix — the brainstem safety tier was under-triaged, then over-triaged, and only right at the fourth version. **Remaining: privacy scan, commit, PR into `develop`, pre-merge smoke (must FAIL), release PR into `main`, deploy smoke.** |
