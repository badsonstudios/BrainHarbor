...
| **In progress** | **nothing mid-flight. WI-526 IS DONE, MERGED AND LIVE.** PR **#117** squash-merged to `develop`, release PR **#118** merged to `main` 2026-09-12, run `34704016126` **green on both jobs (build-test AND deploy) first time.** All 18 live smoke paths 200, and the content verified on the live page itself: "A common belief is that a trial is what you try when everything else has failed", "the first is before the operation that takes the tumor out", the five windows and "Three of those five doors are open early", "does not automatically rule you out", "not designed to find out whether the treatment works", "nobody chooses. Not you, and not your doctor", "you are still getting the standard treatment", "Rarely, and here is the shape of it", "the only way to know is to ask which", "What we cannot tell you, and you should ask", the new "Will I hear what the trial found" paragraph, "Choosing a trial is a medical decision", "Go to the consent conversation", and "the trials we track" — **plus all thirteen inbound doors resolving live**, including the one on the `/trials` finder itself and two each on glioblastoma, high-grade-glioma, astrocytoma and oligodendroglioma (at-diagnosis AND recurrence, which was `/review`'s finding). Verified absent live: any percent sign, "breakthrough", "promising", "last chance", "early access", "Medicare", "you qualify", "petrol", "centre", "most people think", and any matching vendor's name. `develop` and `main` are level; nothing awaits a deploy. **Grade 4.9, 1633 tests, ContentCheck 252/0, 115 break-mutations on LF and CRLF. 104 of 163 done.** Older: **WI-526 (X17 Clinical trials as an option) STARTED 2026-09-12.** Sources fetched and checked FIRST (`.claude/work_files/wi526-sources/notes.md`). **FOUR DOSSIER DEFECTS, and one of them is the framing rule's own citation.** (1) §16 says "NBTS's own framing supports this: patients should take potential trial options TO their health care team", cited to `braintumor.org/.../mytumorid/clinical-trials/` — **that statement is absent from that page**, whose only real sentence is a selling line ("early access to potentially beneficial treatments") that is not printed either. The ask-your-team rule is this site's own policy (WI-449) and IS supported by NBTS, on a different page. (2) **The dossier's phase participant counts are not NCI's** — 20-80/100-300/1,000-3,000 against NCI's "around 15 to 30"/"50 to 100"/"100 to several thousand". Three phases, three disagreements, so no counts are printed; the `[BOUNDARY]` note already recommended that and now has a second reason. (3) **"No one is given a placebo when an effective treatment is available" is not on the NCI page it is cited to**, nor on NCI's safety page. What NCI does say is better: in the common design the placebo is **added on top of** standard treatment, so the reader still gets the standard treatment. (4) **The dossier lost the key nuance in the FRIGHTENING direction:** it says prior treatment excludes people from trials "which is exactly why the conversation should happen before, not after", where NBTS says "**Beginning a new therapy doesn't automatically rule you out from a clinical trial**, but it's important to plan ahead" — and most of this page's readers arrive mid-treatment, so the dossier's version lands on the wrong side for the majority. **The spine is sourced verbatim:** a trial is not a last resort, there are **five windows** of eligibility (NBTS enumerates them), and the moment to ask is at diagnosis. The load-bearing honesty beside it: **a phase 1 trial tests whether the drug is safe, not whether it works** (NCI's own list of what phase 1 assesses does not contain "whether it works"), and **neither you nor your doctor chooses your group** in a randomized trial, so joining is not a way to get the new treatment. **Scope decided:** `/trials` (Razor) does the FINDING and explains the subject in three sentences; the corpus mentions trials **sixteen times across nine pages and explains them nowhere** (six are a bullet in an options list, five are a question in a questions list). This page does the THINKING and routes to `/trials`. **NEVER MATCHING** (WI-449). Costs print the **shape only** — no scheme, no country, no figure — because NCI's costs page is explicitly US-specific and WI-455/457 put non-US readers here deliberately. Older: **nothing mid-flight. WI-525 IS DONE, MERGED AND LIVE.** PR **#115** squash-merged to `develop`, release PR **#116** merged to `main` 2026-09-11, run `34664319313` **green on both jobs (build-test AND deploy) first time.** All 18 live smoke paths 200, and the content verified on the live page itself: "the guidance rather than your team being mean", "they have not broken a rule", "Nobody can give you the odds", "never stop it on your own", "true however short your course", the 988 Suicide and Crisis Lifeline line, the three named enzyme-inducing drugs, the corpus's sudden-deficit ambulance bullet, the confusion bullet, "We do not print a waiting time here", "Bring the dates, not the diagnosis", and the source's own ceiling ("having to come off the medicine altogether because of this is the exception") — **plus all ELEVEN inbound doors resolving live, and the `levetiracetam` tooltip suppressed on the defining page while still firing on `/tumors/low-grade-glioma`.** Verified absent live: any percent sign, the word survival, "It is not common", "fortnight", "licence", "most likely to get it" and "valproate". `develop` and `main` are level; nothing awaits a deploy. Older: **WI-525 WAS CODE-COMPLETE** on `feature/wi-525-anti-seizure-medicines`. `/treatments/anti-seizure-medicines` is a **new page**, the seventh TREATMENT page under the §12.8 LIBRARY template and the sibling of `/treatments/steroids`. Reading grade **5.2**, **1605 tests** (1572 before), ContentCheck **251/0**, **117 break-mutations proven on LF AND CRLF**, **no new glossary terms**, **eleven inbound doors**. **103 of 163 done.** **THE BACKLOG'S HEADLINE CLAIM SURVIVED THE CHECK — the first time in four items.** The Level A recommendation against prophylactic anti-seizure medicine in seizure-naive newly diagnosed patients is verbatim in the **SNO/EANO 2021 practice guideline update** (PMC8563323, Walbert et al); the 2000 AAN parameter it replaces is marked **[RETIRED]** and is not cited. What the check bought was not a correction but the guideline's own **reason** (side effects, interactions, no measurable benefit), which is what turns a refusal into an explanation. **THE SCOPE TRAP IS IN THE SAME GUIDELINE AND CUTS THE OTHER WAY:** peri- and postoperative prophylaxis is **Level C, insufficient evidence**, so a page printing only the Level A tells every reader handed levetiracetam for their craniotomy that their team defied the strongest grade of advice there is. The short-course rule is scoped the same way ("treated surgically **who have not had a seizure**"), and those two readers are indistinguishable from where they stand, so the page hands them a question instead of an answer. **Three dossier defects:** the enzyme-inducing interaction claim is cited to PMC8787304, **which mentions none of phenytoin, carbamazepine, chemotherapy or steroids** (replaced by PMC6657392, which says it outright and adds that it runs BOTH ways — dexamethasone lowers phenytoin levels); the short-course claim rests on a **clinicaltrials.gov protocol PDF**, a plan rather than a result; and "levetiracetam and lacosamide are the most commonly used" overstates a source saying LEV "is still the most widely used". **No frequency is published** for the mood effect — seven figures across seven populations (>13%, 30.1% pediatric, 6.9% having to stop, 14.1%, 13%, 46.4% in TEN people, 58.5% any effect). **The spine gained the half the dossier does not have:** the timing tell (changes start in the days after starting or after a dose change, 24h in the reported case) and that **it reverses fast** when the dose comes down. **`/review` RETURNED FOUR BLOCKERS, THIRTEEN SHOULD-FIXES AND TWENTY-SIX PROVEN GUARD WALK-THROUGHS**, eighteen verified by executing the regex rather than reading it. The blockers: the short version promised the mood change "is fixable" (the body's hedged claim at greater strength, 160 lines earlier, past a mood-scoped guard); **the words "on your own" appeared NOWHERE in the page body** while the guard counting them was satisfied by the title and description alone; the front matter claimed the page printed the source's own ceiling on the risk ("discontinuations are uncommon") and it did not; and **the escalation guard read the shared BLOCK rather than the page**, under a comment saying "This page could have quietly filed them lower" — it had, dropping four symptoms the corpus sorts. **The end-to-end read, thirteenth item running, found eight more**, worst an unsourced **"It is not common"** on the thoughts-of-self-harm warning, plus the caregiver paragraph restating `[CAREGIVER]` ten lines below it in different words and a British "a fortnight ago". **The harness found six guards walking through on BOTH line endings** after the review fixes, including a membership check that a *different* bullet's wording satisfied. **THE HEREDOC BACKSLASH BUG HIT A FOURTH CONSECUTIVE ITEM, inside the item that wrote the rule down** — two `\b`s became backspace characters in the new test file, and it happened twice more in one-line `python - <<` calls. The rule is **any edit whose text contains a backslash goes through Write/Edit or a script file**, and it applies to one-liners exactly as much as to tables. Sibling corrections: `/seizures/what-to-do`'s absolute "Never stop seizure medicine suddenly" (danger kept, scope added) and `/treatments/craniotomy`'s "one of the commonest medicines used to prevent seizures". Ten lessons in §12.8. Older: **WI-525 (X12 Anti-seizure medicines) STARTED 2026-09-11.** Sources fetched and checked FIRST (`.claude/work_files/wi525-sources/notes.md`). **The backlog's headline claim SURVIVED the check for the first time in four items**: the Level A recommendation against prophylactic anti-seizure medicine in seizure-naive newly diagnosed brain tumor patients is real and verbatim in the **SNO/EANO 2021 practice guideline update** (PMC8563323, Walbert et al) — and the guideline's own stated reason (side effects, interactions, no proven benefit) is what turns a refusal into an explanation. **The scope trap is in the same guideline and cuts the other way:** peri- and post-operative prophylaxis is **Level C, insufficient evidence**, so the page must not tell a reader whose team gave them levetiracetam around an operation that their team defied a Level A rule. **Seven dossier/source defects and rulings recorded**, the load-bearing ones: the dossier cites the enzyme-inducing-drug interaction claim to PMC8787304, **which does not mention phenytoin, carbamazepine, chemotherapy or steroids at all** (replacement found: PMC6657392, which says it outright and adds that it runs BOTH ways — dexamethasone lowers phenytoin levels); the short-course-after-surgery claim rests in the dossier on a **clinicaltrials.gov protocol PDF, a plan rather than a result** (the guideline sources it properly, scoped to people who have not had a seizure); **no frequency is published for the mood/temper effect** — seven figures across seven populations (>13%, 30.1% pediatric, 6.9% needing to stop, 14.1%, 13%, 46.4% in ten people, 58.5% any effect); and the dossier overstates "levetiracetam and lacosamide are the most commonly used" against a source that says LEV "is still the most widely used" and LAC is "one of the third-generation drugs recommended". **The spine gained the half the dossier does not have:** the timing tell (changes appear in the days after starting or after a dose change, 24h in the reported case) and that **it reverses fast** when the dose comes down. Budgeted sibling edit, the WI-524 shape one item later: `/seizures/what-to-do` L143 says "**Never stop seizure medicine suddenly**", which is false for the post-operative course the guideline says to stop after a week — corrected to "not on your own" **with the danger kept**. Older: **nothing mid-flight. WI-524 IS DONE, MERGED AND LIVE.** PR **#113** squash-merged to `develop`, release PR **#114** merged to `main` 2026-09-11, run `34657901464` **green on both jobs (build-test AND deploy)** first time. All 18 live smoke paths 200, and the content verified on the live page itself: the title's "might be the drug and not the tumor", "Do not stop a steroid on your own", "no honest number to give you", `The rule is not "always slowly"`, "stops making its own", the lymphoma exception and "cost you the diagnosis", and "opposite things" — **plus all ten inbound doors resolving live.** Verified absent: any percent sign, the word survival, and "twenty-eight" (the only "28" on the page is the ABTA CareLine number in the shell). `develop` is one docs-only commit ahead of `main` (this record). Older: **WI-524 IS DONE** (`/treatments/steroids`, on `feature/wi-524-steroids`). Reading grade **4.7**, **1572 tests** (1542 before), ContentCheck **250/0**, **all 72 break-mutations proven on LF AND CRLF**, **no new glossary terms** (both candidates would have fired nowhere, recorded in the front matter), and **ten inbound doors**. **102 of 163 done.** **THE BACKLOG'S "~28%" IS NOT PUBLISHED AND WAS NOT IN ITS PAPER**: PMC12406498 contains "proximal" zero times and its only 28 is 28.8% of patients with a documented steroid *plan*; the figure is Sturdza 2008 (88 people, brain metastases, palliative radiation, via cns.org), and six other sources say 10%, 2-60%, 10-90%, 10.6%, 60% and 4.5%. The page prints no frequency, says why, and prints the direction all seven agree on. Corrected at source in SYNTHESIS §3.5, content-pipeline §12.6 and the dossier. Three more rulings: **"it does not treat the tumor" is scoped** (false for lymphoma, and the page carries the biopsy-first reason); **"never stop suddenly" became "not on your own"** (a three-day post-op course is stopped outright, per the page's own guideline); and the speed claim §12.14 called unsourced now rests on two guidelines and prints **no improvement share** (two sources disagree and the first draft published the reassuring one). **`/review` found five blockers and walked FOURTEEN OF EIGHTEEN mutations through green** — the description and the title asserted the claims the page exists to refute (`ReaderText` strips front matter, so 24 tests were blind to them), `/treatments/craniotomy` still carried the absolute stop rule four lines above the new door, a fall was filed below the tier three sibling pages give it, "nearly all of them fade" governed bullets that do not, and the headache trigger was scoped to the taper. All eighteen are in the harness table. **The end-to-end read, twelfth item running, found the page explained the taper without ever saying why stopping suddenly is dangerous** — the item's own requirement. **Also fixed on siblings:** radiation-therapy's "**suddenly** cannot get out of a low chair" and its caregiver steroid paragraph (which owned this page's spine), glioblastoma's "that **is** the drug", and two craniotomy sentences plus the two tests that pinned their old wording. The WI-403 Kestrel flake hit once (1572 total, one A11ySmokeTests failure, green on re-run). Older: **WI-523 IS DONE, MERGED AND LIVE.** PR **#111** squash-merged to `develop`, release PR **#112** merged to `main` 2026-09-10, run `34521334233` **green on both jobs (build-test AND deploy)** first time. All 18 live smoke paths 200, and the content verified on the live page itself: "That is the test working", "It may not pass", "This is uncommon", "strongly considered", "less of their tumor taken out", the tooth pain, the two-hour aim, "fair to ask", and the anesthesiologist tooltip firing — **plus all seven inbound doors resolving, the `awake craniotomy` and `brain mapping` tooltips firing on `/treatments/craniotomy` while suppressed on the new page, and craniotomy's "to suggest where" live.** Verified absent live: any percent sign, "survival", "gold standard", "missed out". `develop` is one docs-only commit ahead of `main` (this record). Older: **WI-523 IS DONE** (`/treatments/awake-craniotomy`). Reading grade **4.2**, **1542 tests** (1512 before), ContentCheck **249/0**, **all 101 break-mutations proven on LF AND CRLF**, one new glossary term (`brain mapping`, reachable on `/treatments/craniotomy` through "brain mapping and monitoring"), seven inbound doors (`/treatments/craniotomy` beside its awake mention AND in "Where to go next", plus the surgery bullet of all six glioma hubs). **101 of 163 done.** **The spine had no citation in the dossier** and is now sourced three ways and scoped to the electrical TEST; the removal step says a change then "is not the test. It may not pass". **No seizure number** (five sources, five answers, and the page says why); **"uncommon" printed for the failed awake part**, where the sources agree (about 2%), with its cost and the study's "preventable by adequate patient selection". **`/review` found two blockers**: "you have not missed out if it is not offered" against its own meta-analysis's conclusion ("should be strongly considered" near eloquent areas), and "it comes back" leaking into the removal phase because step 7 called it "testing". Plus 12 should-fixes and 15 guard-walking mutations, all applied. **The suite's frequency guard had an un-interpolated half that could never fire** (a `$@` joined to an `@`), found only by adding a canary. **A shell heredoc put twelve invisible backspace characters into the test file's regexes** and did it again to the mutation table an hour later: any edit containing a backslash goes through Write/Edit. The end-to-end read (eleventh item running) found eight defects, the worst a hospital stay called "much the same" as asleep surgery when both comparing sources say shorter. **Also on siblings:** `/treatments/craniotomy` pre-op scans now "suggest" rather than "map", three explicit anchors (`how-long`, `what-can-go-wrong`, `caregiver`). `/tumors/oligodendroglioma` now reads 5.9 against the 6.0 limit, so its next edit needs headroom. Older: Older: **WI-522 IS DONE, MERGED AND LIVE.** PR **#109** squash-merged to `develop`, release PR **#110** merged to `main` 2026-09-10, run `34498426422` **green on both jobs (build-test AND deploy)** first time. All 16 live smoke paths 200, and the content verified on the live page itself: "It is a plan, not a delay", "Another study found no such difference", "more people ended up having treatment", "an operation should still come first", the seizure half of EORTC 22845, vorasidenib, "sudden, severe and new to you", the one attributed "every few months" range, and the without-a-sample section — **plus all eight inbound doors resolving, and the `watch and wait` tooltip still firing on `/tumors/astrocytoma`**. Verified absent live: 4.26, "fourfold", any percent sign, "survival", and "buys nothing" / "harder to live with than" / "in a way that treatment" on `/tumors/low-grade-glioma`. `develop` is one docs-only commit ahead of `main` (this record). Older: **WI-522 IS DONE** (`/treatments/watch-and-wait`, PR to `develop` from `feature/wi-522-watch-and-wait`). Reading grade **5.2**, **1512 tests** (1477 before), ContentCheck **246/0**, **all 89 break-mutations proven on LF AND CRLF**, eight inbound doors, no new glossary terms. **100 of 163 done.** Two `/review` rounds: 3 blockers + 9 should-fixes + 12 guard-walking mutations, then 4 more should-fixes (the emergency line over-triaged a usual seizure; "the rest never needed it" where many had died of other causes). Also fixed on siblings: `/tumors/low-grade-glioma`'s "buys nothing" and two unsourced watching-is-harder comparatives; EORTC 22845 now one wording on three pages; WI-521's escalation-list guard could not see a list whose every bullet wraps (fixed in both suites). The WI-403 Kestrel flake hit once more (1512 total, one failure in A11ySmokeTests, green on re-run). Older: **WI-522 (X1 Watch and wait), CODE-COMPLETE AND IN THE BREAK HARNESS, 2026-09-10**, on `feature/wi-522-watch-and-wait` (uncommitted). `/treatments/watch-and-wait` is written (grade 5.1, ContentCheck 246/0), `/review` done (3 blockers + 9 should-fixes + 12 guard-walking mutations, all applied), end-to-end read done twice (found the plan-changes contradiction, a team guarantee, an overclaimed link, and the missing vorasidenib option). All content tests green (1112/1112 non-DB). **If this session dies mid-harness: check `git diff` on the content pages for a leftover mutation and delete `.break-tests-running` before re-running** (`python .claude/work_files/wi522-sources/break-tests.py`, 86 mutations). Remaining: harness green on LF+CRLF, one full suite run with the database, commit/PR/merge/release. **THE ITEM'S RULING: the backlog's 4.26x depression figure is NOT published** — univariate odds ratio, 31 v 31, abstract mislabels it multivariate, the paper concludes distress is high "independent of management strategy", three other studies found no difference. **Environment note:** a Macrium backup (`ReflectBin`) saturated the disk this session; Npgsql timeouts and a hung testhost were that, not the change. Older: **WI-522 STARTED 2026-09-10.** Sources fetched and checked first (`.claude/work_files/wi522-sources/notes.md`). Older: **WI-521 IS DONE, MERGED AND LIVE.** PR **#107** squash-merged to `develop`, release PR **#108** merged to `main` 2026-09-09, run `34414607684` **green on both jobs (build-test AND deploy)** first time. All 16 live smoke paths 200, and the content verified on the live page itself: the summary covering **both** look-alike windows, "inside an agreed band", the booking ask, "not the same as saying nothing is there", "no reliable figure to put here", "It is common.", "primarily intended for clinical trials", both directions of stable, the door out of **progression**, "they do not settle it", the radiation-necrosis scope ("one kind of radiation, and it may not be yours") with stereotactic radiosurgery named, "None of them reduced it", the scanxiety scope ("after cancer surgery"), and the confirmation-scan route — **plus all nine inbound doors resolving and `/tumors/high-grade-glioma` now carrying RANO's third route live.** Verified absent on the live page: any percent sign, the word survival, any scan interval, and any caregiver block. `develop` and `main` are level; nothing awaits a deploy. Older: **WI-521 WAS CODE-COMPLETE** on `feature/wi-521-follow-up-scans`. `/tests/follow-up-scans` is a **new page**, the tenth under the §12.8 **LIBRARY** template and **the third in the corpus to use all twelve slots** (after WI-506 and WI-510). Reading grade **5.2**, **1477 tests** (1444 before), ContentCheck **245/0**, **all 72 break-mutations proven on LF AND CRLF**, **no new glossary terms**, and **nine inbound doors** so the page is not an orphan (`/tests/mri`, `/tests/waiting-for-results`, `/treatments/radiation-therapy` and all six adult glioma hubs). **THE ITEM'S RULING IS THAT THE PAGE PUBLISHES NO FREQUENCY AT ALL** — the backlog asked for "10-30% of glioblastoma patients", the dossier says "20-30% ... range 12% to 64%", the ASCO Post says "28% to 66%", PMC10412732 says "approximately 36%" and separately reports a biopsy series at **12.4%**. Five answers, none of them each other, because each counted a different thing, so §12.4 R2's reasoning applies to a **frequency** and not only to a procedural risk. The page says it is common **and says why there is no number**, because a page that is silently silent leaves the reader to supply their own. **`/review` FOUND THE PAGE SHARED 108 EIGHT-WORD SHINGLES WITH `/tumors/high-grade-glioma`** — whole paragraphs of pseudoprogression and radiation-necrosis prose lifted wholesale, already drifted inside one commit ("dead tissue" there, "damaged tissue" here) — **while the restatement guard, scoped to three hand-picked tests pages, reported zero.** Both sections are rewritten in this page's own voice; the guard now runs **corpus-wide**, strips headings and link targets (slot 9's heading is identical on all 29 library pages by prescription), requires three content words per shingle, and carries a **four-entry allowlist** of deliberately-shared safety claims. **`/review` then walked eleven more guards by mutation** — "one in three people", "Half of the people who", "Most people having chemoradiation", "every few months", "three months apart", "twice a year", a `###`-headed escalation list, an escalation list with a sentence between the lead-in and the bullets, "it shrank by at least half", "a perfusion scan usually identifies the difference", and a breathing exercise that "calms the waiting". Every one is now in the mutation table and fails on both line endings. **THE END-TO-END READ CAUGHT ITS NINTH DEFECT IN A ROW, and this time six of them:** the page never said **what a scan cannot do** (§12.8's own WI-506 rule); a reader whose report says **progression** reached the bottom with nowhere to go; *"anyone who gives you one is guessing about you"* undercut the one authority the page keeps telling them to ask; *"they are not allowed to give you a result"* was a policy claim no source supports where a role fact was available; *"your team **will** start again from the top with you"* guaranteed somebody else's behaviour next to the hedged version of itself; and one sentence had been edited into nonsense (*"the rules your team works to build a step in for it"*) which grade 5.3, ContentCheck 245/0 and 1,477 tests all passed. **Four more dossier defects, recorded in the front matter with replacements:** §10.4's radiation-necrosis timing is "median 7 months, range 1-25" where PMC8507553 says **8 months, range 1-41**, in **stereotactic radiosurgery** patients (15.8% metastases, 34.6% meningiomas, 4.8% schwannomas) rather than glioma chemoradiation; §10.4's "imaging matched biopsy in all cases" is attributed to that same paper, which states **biopsies were never performed**; §10.5's scanxiety figures ("over 70%", "71.2%") are **absent from both open sources**, which report 34-85% across 22 studies of people followed after **curative-intent cancer surgery**; and §10.2 calls RANO 2.0 "four categories" when the reachable paper's table has **five**. **Deliberate omissions, each with a test and a reason:** no surveillance schedule (its source is 403, one schedule is wrong for most of this page's readers, and it is a number a frightened reader **can** act on); no radiation-necrosis incidence; no RANO thresholds; **no escalation list** (what to call about between scans is tumor-specific); **no caregiver section** (a follow-up scan has no aftercare tail, and `/tests/mri` and `/tests/waiting-for-results` carry none either). **Also fixed on a sibling:** `/tumors/high-grade-glioma` listed only two of RANO's three routes out of the twelve-week rule and omitted the **mandatory confirmation scan**, the most reassuring one. **Known and pinned rather than hidden:** the `rano` glossary entry is reachable **nowhere** — this is the first page in the corpus to use the word in prose and it defines it, so the tooltip is suppressed here; a test holds that state and goes red when a later page makes it live. **The WI-403 Kestrel flake hit locally TWICE during this item** (`A11ySmokeTests.TheReaderChoiceGateOpensAndClosesWithJavaScriptDisabled`, 1477 total both times with one failure, green on re-run both times). It is now the third item running where it has appeared, so treat "one failure in A11ySmokeTests with the full count" as the signature and re-run before investigating. **99 of 163 done.** Older: **WI-521 STARTED 2026-09-09** — sources fetched and checked FIRST, and the dossier (`tests-library.md` §10) is wrong in five places, three of them on numbers the backlog asked the page to publish:** (1) §10.3 says pseudoprogression is "20-30% ... range 12% to 64%" — the ASCO Post says **28% to 66%** and PMC10412732 says **~36% in a recent meta-analysis** with a biopsy series at 12.4%; the backlog's own "10-30%" matches none of them, so **no percentage goes on the page** (R2's reasoning: the spread is about what each study counted); (2) §10.4's radiation-necrosis timing "median 7 months (range 1-25)" is **not what PMC8507553 says** — that paper says **median 8 months, range 1-41**, and its 15.7% is **SRS for intracranial tumors**, mostly metastases and meningiomas, not glioma chemoradiation; (3) §10.4's "in one small biopsy series imaging diagnosis matched biopsy in all cases" is attributed to that same paper, which states **biopsies were never performed**; (4) §10.5's scanxiety figures ("over 70%", "71.2% in one prospective longitudinal study") are **absent from both open sources** — PMC11659363 reports **34-85% across 22 studies and 8,693 patients** and calls it "nearly universal"; (5) §10.2 calls RANO 2.0 "four categories" when the reachable paper's table has **five** (CR, PR, **MR**, SD, PD), and both its citations (ascopubs.org, academic.oup.com) are on the known-dead list. Reachable replacements fetched: **PMC12479667** (RANO 2.0 — baseline is the scan **21-35 days after radiation ends**, mandatory confirmation scan at **4 or 8 weeks** for new enhancement inside 12 weeks, T2/FLAIR removed, and the verbatim admission that the criteria are "**primarily intended for clinical trials**"), PMC10412732, the ASCO Post pseudoprogression review ("cannot reliably distinguish"), PMC8507553, PMC11659363 (**"none led to reduced rates of scanxiety"**, and intensified surveillance made no difference), PMC10000102, and the ACS follow-up-care page. Older: **WI-520 IS DONE, MERGED AND LIVE.** PR **#105** squash-merged to `develop`, release PR **#106** merged to `main` 2026-09-09, run `34302818858` **green on both jobs (build-test AND deploy)**. All 12 live smoke paths 200, and the content verified on the live page itself: the fasting caveat **and** the blood-thinner rule both in the short version, the two-hour rule with its scope rendering **above** the sentence that invites the pushback, "no alcohol", the European attribution in the same sentence as the twelve-hour figure, "including this page" in both the section and the caregiver half, the ECG hedge, "There is no fixed list of tests", "do not assume either way", and the steroid taper rule — **plus all four inbound doors resolving and the `blood thinner` tooltip firing on `/tests/biopsy` and `/treatments/craniotomy` while staying suppressed here.** Verified absent live: any per-drug day count, the fatty-meal tier, and any ambulance list of this page's own. `develop` and `main` are level; nothing awaits a deploy. Older: **WI-520 WAS CODE-COMPLETE** on `feature/wi-520-getting-ready-for-surgery`. `/tests/getting-ready-for-surgery` is a **new page**, the ninth under the §12.8 **LIBRARY** template. Reading grade **5.2**, **1444 tests** (1415 before), ContentCheck **244/0**, **all 55 break-mutations proven on LF AND CRLF**, three new glossary terms (anesthesiologist, advance directive, blood thinner) and **four inbound doors** so the page is not an orphan (`/start`, `/tests/biopsy`, `/tests/mri`, `/treatments/craniotomy`). **98 of 163 backlog items done.** **THIS IS THE FIRST PAGE IN THE CORPUS THAT PUBLISHES AN INSTRUCTION A READER CAN FOLLOW TONIGHT** — a fasting interval and a blanket blood-thinner prohibition — and almost everything below is about holding those two open. **`/review` RAN SIXTEEN MUTATIONS AND THIRTEEN WENT THROUGH GREEN, SIX AGAINST TESTS NAMED FOR THE DEFECT BEING REINTRODUCED.** A four-literal ban on "steroids are generally continued" was beaten by the **singular**; a day-count guard requiring `N days (before|prior)` was beaten by **"five days ahead"**, then by a drug not on its list, then by the **questions list it never read**; and the age regex written to catch the dossier's own "above roughly age 50-60" **could not match it**. All thirteen are now in the mutation table and fail on both line endings; the tests are claim-shaped rather than string-shaped. **THE END-TO-END READ CAUGHT THE §12.14 DEFECT AGAIN, ON THE ITEM THAT WROTE THE BAN** — the medicines section opened "Most of what you take, you keep taking", which is the banned MRI page's "take your regular medications as usual" in my own words, three tests below the ban. **The eighth consecutive item where the human-style read found what the suite could not.** `/review` also found the fasting permission **escaping its caveat in the two places most readers meet it** (the short version, which the section-scoped position test structurally could not see and which the summary test actively **pinned in its defective form**, and `/start`'s "when you really have to stop eating and drinking"), and that the **two-hour rule was printed without the scope its own source attaches to it** — "for elective procedures", "healthy patients", in a paper mostly about GLP-1 agonists and gastroparesis — while the section arms the reader to push back on a longer time. Both fixed. **Deliberate omissions, each with a test and a reason:** no per-drug antiplatelet stop-day counts (they are real and correctly sourced, and they stay off because a frightened reader **can follow them**, next to a heading saying not to — the page keeps one number, "around a week", and it only pushes toward asking sooner); no eight-hour fatty-meal tier (**no reachable adult source states it**); no restart timings; **no escalation list at all** (the reader has no symptoms yet, and a partial list is a tier-diff liability with no reader benefit). Two site-wide guards fired on the new content and both were right: an em dash, and "slow" in the blood-thinner definition — reworded rather than exempted. The **WI-403 Kestrel flake hit locally once** during this item (1444 total, one failure, green on re-run). **§12.8 gained nine lessons**, including the `[CAREGIVER]` "for this tumor" question WI-519 deferred, now **decided: it stays**, because this reader has a planned operation and therefore a diagnosis. Older: **WI-520 STARTED 2026-09-09.** Sources fetched and checked first, and the dossier (`tests-library.md` §6) is **wrong or unusable in six places**: (1) §6.4 attributes the whole ASA fasting table to `guidelinecentral.com/guideline/8964/`, which contains **no fasting guidance at all** — the page's headline claim had no source; (2) §6.5's "dexamethasone and anti-seizure medicines are generally continued" is attributed to `radiologyinfo.org/en/info/mri-brain`, an **MRI page** that mentions neither, whose only medication sentence is about an MRI; (3) §6.2's "ECG ... usually indicated ... above roughly age 50-60" is **contradicted by its own co-citation** — the ASA advisory says "**Age alone may not be an indication for ECG**"; (4) `accessanesthesiology.mhmedical.com` is **403**, cited twice; (5) §6.2's group-and-save / coagulation / endocrine-bloods paragraph carries **no citation at all**; (6) §6.5's anticoagulant restart timings are unconfirmed and post-operative anyway. Replacements found and fetched: PMC13436883 (ASA 2023 two-hour clear-liquid rule verbatim, **the Thirst study** — 46 centres, 12 European countries, n=5100, median clear-liquid fast **12 hours**, fewer than 1% drinking within two hours), PMC12524845, PMC11701341, PMC5470849 (anti-seizure medicines "maintained ... as close to baseline as possible", morning dose "with a sip of water"), PMC11451960 (abrupt steroid stop -> adrenal crisis), The Hospitalist's open summary of the 2024 ACC/AHA perioperative guideline, MedlinePlus's **Medical Test** page for the ECG (**not** an AHFS drug monograph, so PLAN.md §5 does not reach it), RadiologyInfo chest x-ray, ACS and NBTS. Older: **WI-519 IS DONE, MERGED AND LIVE.** PR **#103** squash-merged to `develop`, release PR **#104** merged to `main` 2026-09-09. **The first run FAILED and the deploy was SKIPPED** — `A11ySmokeTests.TheReaderChoiceGateOpensAndClosesWithJavaScriptDisabled` hit WI-403's *"The Kestrel test host did not start"*, the intermittent PROGRESS has recorded as **not proven fixed** since WI-403. **This is the first time it has been seen in CI rather than locally.** It is not contention of the WI-517 kind: the class carries `[Collection(DatabaseCollection.Name)]` and `TestCollectionHygieneTests` is green. Note the run reported **1413 tests, not 1415** — when the Kestrel host fails to come up, the rest of that class does not run, so **a lower count is the signature of this flake rather than of a stale assembly**. Re-running the failed job on the same commit came back **green on both jobs (build-test AND deploy)**. All 14 live smoke paths 200, and the content verified on the live page itself: "It is not a treatment", "not an attempt to remove the tumor", "You must not drive after a brain biopsy", the corrected bleeding causes ("a small target", "bleeding during the procedure itself"), the added ambulance symptom, the overnight adult, "burr hole" and the consent-form sentence — **plus all ten inbound doors resolving on the live site.** `develop` and `main` are level; nothing awaits a deploy. Older: **WI-519 WAS CODE-COMPLETE** on `feature/wi-519-biopsy`. `/tests/biopsy` is a **new page**, the eighth under the §12.8 **LIBRARY** template and the first written after three §12.3 hubs in a row. Reading grade **4.4** (the lowest of any P5 page), **1415 tests** (1388 before), ContentCheck **237/0**, **all 51 break-mutations proven on LF AND CRLF**, all twelve slots plus a caregiver section, and **ten inbound links** added so the page is not an orphan (`/start`, `/tests/mri`, `/tests/waiting-for-results`, `/treatments/craniotomy` and all six adult glioma hubs). **No new glossary terms**: a `burr hole` entry was written and withdrawn, because the page defines the word so the tooltip is suppressed here, and no other page uses it — the entry would have fired nowhere. **THE BLOCKER WAS MINE AND NO GATE COULD SEE IT: the first draft had no risk section at all.** The page described a needle going into someone's brain, told the person driving them home to call an ambulance if they suddenly could not speak, and never said what could go wrong or why. Found by the end-to-end read of the rendered page — the **seventh** consecutive item where that read caught what the suite could not. **`/review` then found three blockers and ten should-fixes, and PROVED four of them by mutation.** (1) The risk section blamed bleeding on **where the tumor sits**, imported from `/treatments/craniotomy` where location is the story; the cited paper concludes **lesion size** and intraoperative bleeding, and the wrong cause was propping up a reassurance. (2) The driving prohibition was **narrowed** from the source's "you must not drive after having a brain biopsy" to "you must not drive **yourself**", filed under a packing list and never reaching the caregiver section. (3) `TheAmbulanceListDoesNotOverEscalateASeizureAgainstTheSeizurePage` **could not fail on the defect it is named after** — review flattened the bullet to "A seizure", WI-510's blocker verbatim, and eighteen tests stayed green. (4) The tier check was **one-directional**, so three symptoms craniotomy carries were silently missing here. Also: the caregiver-duplication guard was the **bold lead-in** check §12.8 already calls insufficient while the shingle version existed in three sibling files; and the lab-pipeline guard was five nouns that a genuine restatement walked around. **Two dossier citation defects:** §4.1's "a meta-analysis found no significant difference in diagnostic yield" rests on `surgicalneurologyint.com` (a **24-word JavaScript shell**) and on PMC10219353, which is **not a meta-analysis** but a 72-patient single-centre comparison — the test bans the URL **and the word**; and §4.3 bundles "a small shaved patch" into a bullet cited to ACS, which contains "shav" **zero** times, so the page asks the question instead. §12.13 and §12.14 were also **reordered in the doc**, which had them backwards since WI-518. **97 of 163 backlog items done.** Older: **WI-518 IS DONE, MERGED AND LIVE.** Release PR **#102** merged 2026-09-08, run `34284021680` **green on both jobs (build-test AND deploy)**, all 12 live smoke paths 200, and the content verified on the live page itself: the growth story, "starve and die", "immature, badly formed", "go by the IDH line", bevacizumab and Optune all present, **and `/tumors/high-grade-glioma`'s corrected honesty note rendering scoped to diffuse midline glioma**. `develop` is one docs-only commit ahead of `main` (the WI-564 amendment); it rides along with the next release. **ALL SIX ADULT GLIOMA HUBS ARE NOW WRITTEN AND LIVE**: glioma, low-grade, high-grade, astrocytoma, oligodendroglioma, glioblastoma. Older: **WI-518 IS DONE** (`/tumors/glioblastoma`; PR open to `develop`). A 230-word stub became the §12.3 seventeen-section hub. Reading grade **5.8**, **1388 tests** (1358 before), ContentCheck **236/0**, **all 42 break-mutations proven on LF AND CRLF**, two new glossary terms. **96 of 163 backlog items done.** **The engine had no reachable source:** the dossier hangs the entire growth story on `academic.oup.com/jnen` (403, third item to find that domain closed); rebuilt from PMC12564729 + PMC2588896 + NBK537272, with the two spatial claims that belonged only to the dead source softened rather than kept. **`/review` found five blockers.** The worst was §12.12's defect mirrored: the short version told every holder of *secondary glioblastoma* that their diagnosis has a different name today, which is **false for the IDH-wildtype ones — and `/tumors/astrocytoma` routes those readers HERE.** Two hubs disagreeing about who belongs where, with the wrong one in the higher-traffic position. **Two more claims shipped in prose whose only source this item's OWN TEST bans** (steroid speed, local recurrence) — **banning a citation is not fixing a claim, and the ban makes it look handled.** Written up as **§12.14**. **One finding was REJECTED with evidence:** the shorter-radiation paragraph was called an unsourced comparative off the dossier's paraphrase; the paper itself says "the preferred standard of care ... comparable overall survival" and EANO says "similar activity", so the page follows the sources and the verbatim is now in the front matter. **The honesty note on `/tumors/high-grade-glioma` had become false** — three of four destinations are filled in, so a reader was told the glioblastoma page was thin and pointed back to high-grade-glioma as "the fuller one". It now names diffuse midline glioma specifically; **WI-535 retires it.** Older: **WI-517 IS LIVE.** Release PR **#100** merged 2026-09-08, run `34277558672` **green on both jobs (build-test AND deploy)**, all 12 live smoke paths 200, and the content verified on the live page itself: 6,591 words (was 172), the both-halves opening, the corrected "unusually likely to respond to chemotherapy", "no oligodendroglioma that corresponds to grade 4", the retired-name slice with the eliminated/renamed distinction, CODEL, and the `blocks/mechanism.md` idiom fix showing on `/tumors/astrocytoma` too. **`develop` and `main` are level; nothing awaits a deploy.** Older: **CI CAUGHT A REAL RACE THAT HAD BEEN LIVE SINCE WI-514, AND THE FIRST WI-517 RELEASE FAILED ON IT.** Release PR #99 merged but run `34276911369` failed: **397 test failures, none about content** — `23505: duplicate key value violates unique constraint "pg_database_datname_index"`, which is two `DatabaseFixture` instances running `CREATE DATABASE` at the same moment. The deploy job was **skipped**, so the site stayed on the previous build and the smoke paths all returned 200 while serving old content. **A green smoke check does not mean the deploy happened — check the run's conclusion and the deploy job, not just the HTTP codes.** **Cause:** three render classes (`AstrocytomaPageRenderTests`, `GliomaPageRenderTests`, `HighGradeGliomaPageRenderTests`) shipped without `[Collection(DatabaseCollection.Name)]` across WI-514 to WI-516, so they ran in parallel with every other database-touching class. Nothing noticed until WI-517 added a fourth render class and made the race likely. **Locally this read as flake** — a different test failed on each full run and every one passed in isolation. **A suite that fails somewhere else each time it runs is not flaky, it is contended.** **Fixed and guarded:** all three attributed, plus `TestCollectionHygieneTests`, which finds any class standing up a web host without the attribute **by reflection** rather than a text scan, and was proven by removing an attribute and watching it go red. **1358 tests green twice in a row.** Release PR **#100** re-ships WI-517. Older: **WI-517 IS DONE** (`/tumors/oligodendroglioma`; PR open to `develop`). A 172-word stub became the §12.3 seventeen-section hub. Reading grade **5.8**, **1357 tests** (1322 before), ContentCheck **232/0**, **all 47 break-mutations proven on LF AND CRLF**, two new glossary terms. **95 of 163 backlog items done.** First hub written *against* `[ESCALATION]` rather than hand-copying it. **§12.1 cost the most here and it is now §12.13:** the richest source on this tumor is the StatPearls oligodendroglioma chapter, the exact chapter §12.1 bans for naming and grading — cited for symptoms and treatment mechanics only, with the naming claims on CNS5-aligned sources. **Three dossier claims are absent from the sources they cite**, and one of them is flagged in the dossier as *"say this, because it is reassuring and true"* — **that parenthetical is the tell.** **`/review` found FIVE blockers.** The worst was a **whole missing block**: no retired-name crosswalk slice, on the one hub whose central fact IS a retired name (oligoastrocytoma was **eliminated**, and the 1p/19q test this page explains is what eliminated it) — **and the test guarding it iterated over regex MATCHES, so on a page with none the loop never ran and it passed.** Second: the headline claim **overstated its source by one word** ("prediction of the best drug RESPONSE" became "predicts WHICH DRUGS"), which is drug selection rather than responsiveness, and contradicted the "genuinely open" section three screens below. Also a fabricated caregiver population claim, two superlatives out of one source's "relative", and a grade-2 radiation-timing conversation offered to grade-3 readers. **The break harness then found six more, four of them real test weaknesses** — including a §12.6 landing check that read the last sentence of a section with three subsections under it and **could never fire**. **Also fixed beyond this page:** `blocks/mechanism.md` said "feeling sick and being sick", the same idiom WI-563 fixed one item earlier, on the block that composes onto every hub. Older state below. **WI-517 (oligodendroglioma) STARTED. WI-563 IS DONE, MERGED AND LIVE** — PR #96 to `develop`, release PR **#97** to `main` merged 2026-09-08, run `34271129026` green, **all 13 live smoke paths 200**. Fourth clean deploy window in a row. **Verified live: `/tumors/glioma` now renders the fever rule twice** (chemotherapy and post-surgery) on a page that carried the word "fever" **zero** times an hour earlier. `develop` and `main` are level; nothing awaits a deploy. Older: **WI-563 IS DONE** (`[ESCALATION]` shared block; PR open to `develop`). **1322 tests** (1311 before), ContentCheck **228/0**, **29 break-mutations proven on LF AND CRLF**, hub grades unchanged (low-grade 5.3 -> 5.2). Four pages now include the block. **94 of 161 backlog items done** (WI-564 and WI-565 added as follow-ups). **The item's own plan was wrong and the item reversed it:** the backlog entry said the tiers were universal and the fever line was page-local routing — checking that found `/tumors/glioma` linking `/treatments/chemotherapy` with the word "fever" **zero** times, and `/tumors/low-grade-glioma` mentioning chemotherapy four times with "fever" only in a link label. **Two live pages carrying §12.11's defect**, the one WI-515 was blocked for nearly shipping. A *conditional* is true on every hub and false on none, so both fever rules (chemo and post-surgery) went into the block. **`/review` found two blockers:** *"Being sick over and over"* is British for vomiting and reads as *being unwell* to a US reader, on a same-day escalation trigger in the highest-blast-radius file on the site; and factoring the three page-level tests into one helper **silently dropped the section scoping all three had**, so relocating `[ESCALATION]` to the bottom of a page kept the suite green (WI-512's defect, re-committed by the factoring that cites it). **Three of my own checks were theatre:** the craniotomy cross-check terminated on a string that page does not contain (capture ran to EOF), compared "vision" against a page that says "cannot ... see", and the chemotherapy check matched that page's **intro** rather than the `{#fever-rule}` section its link points at. **And one self-inflicted scare: a trim script cut to end of file and deleted the two MRI test classes — caught only because the test COUNT went DOWN to 1308 while the suite stayed GREEN.** Read the count, not just the colour. Older state below. **MODE CHANGE 2026-09-08 — UNATTENDED. DAN HAS STOOD DOWN BOTH APPROVAL GATES AND THE PRE-DEPLOY LOCAL READ.** His words: *"I'm not going to review these anymore. I just want you to go ahead and let's just get through this process and get all these done, and stop waiting for me."* This **retires the standing rule** that he read every reader-facing page at `localhost:5177` before a deploy (memory `test-locally-before-deploy`, deleted; replaced by `run-autonomously-no-review-gate`). Run `/next-item` back to back with Gate 1 and Gate 2 replaced by self-checks, merge to `develop`, and batch release PRs to `main` without asking. **The automated guardrails are now the only reviewer** — ContentCheck, the ≤6.0 reading level, `/review`, and the break-test harness on LF **and** CRLF — so none of them may be skipped or softened, and the human-style end-to-end read of the page stays mandatory (it has caught a defect the suite could not on five separate items). He also asked for a **paste-ready next-session prompt** whenever context grows large, before he clears (memory `handoff-prompt-before-clear`). **WI-516 IS NOW LIVE** — release PR **#95** (`develop` -> `main`) merged 2026-09-08 without a local read, per the above. **NEW ITEM ADDED BY `/pm`: WI-563, the `[ESCALATION]` shared block, slotted BEFORE WI-517** — the twelve-line ambulance/same-day list is byte-identical across three hubs (glioma L200, high-grade-glioma L205, astrocytoma L191) and the fever sentence has already diverged between two of them ("explains why, and what number your team will give you" vs "explains why."). Backlog is now **93 of 161**. Correction to the earlier count of four hand-copies: `/tumors/low-grade-glioma` carries a **compressed one-paragraph** version, not the twelve lines, and the three `/treatments/*` lists are procedure-specific and are not this block. Older state below. **WI-516 IS DONE AND MERGED** (PR #94, squash-merged to `develop` 2026-09-08; `develop` head `8225483`). `/tumors/astrocytoma`: a 182-word stub is now the §12.3 seventeen-section hub, **4,129 words**. Reading grade **5.7**, **1311 tests** (1275 before), ContentCheck **228/0**, **all 77 break-mutations proven on LF AND CRLF**, one new glossary term (astrocyte) plus a `CDKN2A/B` alias that makes an existing entry reachable. **93 of 160 backlog items done.** Dan read the page locally before approving, so the pre-deploy rule holds for the third item running. **NOT YET DEPLOYED: `develop` is one item ahead of `main` and no release PR is open** — the next release ships WI-516. `/review` found **two blockers and both were mine, and both were over-reassuring**, which §12.12 now records as the more dangerous direction. Older state below. **WI-515 IS DONE, MERGED AND LIVE** (PR #92 to `develop`, PR #93 to `main`). `/tumors/high-grade-glioma`: a 194-word stub is now the §12.3 seventeen-section hub. Reading grade **5.6**, **1275 tests** (1233 before), ContentCheck **226/0**, **all 91 break-mutations proven on LF AND CRLF**, two new glossary terms (chemoradiation, PCV). **92 of 160 backlog items done; 68 items remain, 44 of them tumor-guide pages.** No feature PRs open. **DAN READ THE PAGE LOCALLY AT `localhost:5177` BEFORE APPROVING THE COMMIT** — the standing pre-deploy rule (memory `test-locally-before-deploy`) is back in force after two releases that skipped it. **WI-515 IS LIVE.** Release PR **#93** (`develop` -> `main`) merged 2026-09-08, run `34172457708` green on both jobs. **Live smoke: `/`, `/tumors/high-grade-glioma`, `/tumors/glioma`, `/tumors/low-grade-glioma`, `/treatments/chemotherapy`, `/tests/molecular-markers`, `/research`, `/trials`, `/search`, `/get-help-now`, `/glossary` — ALL 200**, including the four that returned empty 500s in four earlier deploy windows. Two clean windows in a row now. Content verified live on the page itself: the scoped curability sentence, the no-grade-4-oligodendroglioma fact, pseudoprogression, the fever rule and the closed outlook gate. **This release had the local read** — Dan read the page at `localhost:5177` before approving the commit, so the rule in memory `test-locally-before-deploy` is honoured again. **NOTHING IS AWAITING A DEPLOY: `main` and `develop` are level and no release PR is open.** Older: **RELEASE PR #90 IS MERGED AND DEPLOYED** (2026-09-07, on Dan's instruction; merge commit, `develop` not deleted). Run 34164795315 green on both jobs; live smoke `/`, `/tumors/glioma`, `/tumors/low-grade-glioma`, `/treatments/radiation-therapy`, `/treatments/chemotherapy`, `/tests/molecular-markers`, `/research`, `/trials`, `/search`, `/get-help-now` **all 200** — including the four that 500'd in the last four deploy windows — and the outlook gate renders on the live glioma page. **WI-509 THROUGH WI-514 ARE NOW ON `main` AND LIVE.** **That release also shipped without the local read** (memory `test-locally-before-deploy`), the second in a row. Dan was offered the localhost review and chose to merge. **Recorded as a departure again, NOT a precedent** — two in a row is the point at which it starts looking like the rule is dead, and it is not. Older state below. WI-514 IS DONE AND MERGED (PR #91, `develop` head `2568b0d`). 1233 tests green, ContentCheck 223/0. Wave 2's first item, first hub written to §12.9 rather than proving it. Turns `/tumors/glioma` (today a **166-word, 4-section stub**) into the §12.3 seventeen-section hub, and writes the two shared blocks it is canonical home for: `blocks/crosswalk.md` and `blocks/mechanism.md`, neither of which exists yet. Also on this item: reconcile `/tumors/low-grade-glioma`'s inline crosswalk slice (lines 73-85) against the new shared block so the Roman-numeral line is not said twice. **ALSO OPEN: RELEASE PR #90 (`develop` -> `main`) IS OPEN AND UNMERGED — merging it IS the Azure deploy** of WI-509 through WI-513 in one window. Opened 2026-09-07 on Dan's explicit instruction. **WI-513's TEMPLATE IS SIGNED OFF (2026-09-07) — Wave 2 is unblocked.** Both decisions were made **sight-unseen**: the site was started at `localhost:5177` and the three review pages returned 200, but Dan approved before reading them, knowingly setting aside the standing pre-deploy review rule (memory `test-locally-before-deploy`). Recorded because it is a departure, not a precedent — the rule still stands for the next release. P5 Wave 1 is complete and merged into `develop`; PRs **#86 (WI-511), #89 (WI-512, replacing #87), #88 (WI-513)** all squash-merged. `develop` head is `aebe36c`, **1195 tests green, ContentCheck 219/0**. The hub template is written down at `docs/content-pipeline.md` **§12.9** (new section, separate from §12.8, which is the LIBRARY template). **A stacked-PR lesson worth not repeating:** `gh pr merge --squash --delete-branch` on the bottom of a stack **auto-closes the PR above it** (GitHub closes a PR whose base branch is deleted, and a closed PR cannot be reopened or retargeted). #87 was lost that way and had to be re-created as #89. Squash-merging also means the branch above must be rebased with `git rebase --onto origin/develop <last-commit-of-the-merged-item>` — using develop's squash commit as the upstream replays the already-merged work and conflicts. Everything below this sentence is older state. 
| **Next up** | **WI-527 (Meningioma, deepened)** — **Wave 3 starts here**, and the first hub outside the glioma family, so it uses **§12.3's seventeen-section order** (the TUMOR HUB template) and not §12.8's twelve-slot library template: the two are easy to confuse and the last five items were all library pages. Depends on WI-513, which is done. Spine: **it starts on the covering, not in the brain**, and the CSF cleft on the scan is the visible proof — genuinely reassuring, and explained nowhere — with the honest limit that compression still causes real damage. **Location matters more than size.** Why so many are found by accident; grade 1-3 and what "atypical" changes; defusing "brain invasion" on a report. **THE HORMONE FINDINGS COME AT THREE DIFFERENT EVIDENCE LEVELS AND MUST NOT BE MERGED INTO "HORMONES CAUSE MENINGIOMA":** high-dose progestogens are accepted as causal by European regulators and must be stopped on diagnosis; HRT is mixed; **standard combined oral contraceptives do not increase risk**. That is §12.12's shape three times over in one section, and the over-frightening direction is the live risk — a reader on the pill who reads a merged claim stops a medicine for no reason. **Read `docs/content-pipeline.md` §12.3 AND §12.9 first** (the hub template and its proof), then §12.8's lessons, which are template-independent. **WI-526 added nine**, and the four that matter most here: **a one-sentence correction can over-shoot into a different contradiction** (fix against the page, not the sentence — WI-526 needed three passes on one claim); **emphasis markers and line wraps defeat a phrase guard structurally**, so run phrase guards over text with `[*_]` stripped; **a pattern escape is a door and a short named allowlist is not**; and **pinning a sentence's first clause leaves its tail free**, so pin whole sentences end-anchored. Also: **count list items by marker, not by numeral**, and **a door needs to be near the sentence it was appended to** or the guard proves nothing. `[ESCALATION]` applies but §12.10 warns it is **not automatically right for a hub whose emergencies differ** — a meningioma's worst acute events are not a glioma's, so diff the tiers in both directions before including the block. Still live: `/tumors/high-grade-glioma`'s honesty note (WI-535 retires it); `TheRanoGlossaryEntryIsReachableNowhere...` still pins RANO as used nowhere; WI-564 and WI-565 logged, not blocking. **One `/pm` item WI-526 surfaced:** `/treatments/radiation-therapy` L308 and L549 and `/tests/waiting-for-results` L254 name "your insurance", which the jurisdiction standard WI-526 set (and WI-455/457/560) says to replace with "whoever normally pays for your care". Older: **WI-526 (X17 Clinical trials as an option)** — how to think about them, framed as **"ask your team"**. **NEVER MATCHING**: Leal Health and Massive Bio do AI trial matching, and that is a regulated space this site stays out of (WI-449). Depends on WI-502, which is done. **This site already has a `/trials` page fed by ClinicalTrials.gov** — read it first and decide what this page owes it that it does not already do, because a second page about trials that restates the first is the §12.10 defect with a whole page's blast radius. **Read `docs/content-pipeline.md` §12.8 first — WI-525 added ten lessons**, and the three that matter most here: **a guideline can contain its own scope trap** (read the whole recommendation list, not the line the backlog quoted — WI-525's Level A and Level C were three recommendations apart and a page carrying one without the other accuses somebody's team); **a test can be satisfied entirely by the front matter** (`Everything` is title + description + body, so assert properties against `Body` and use `Everything` only for bans); and **check the AGENT, not the modal** — a sentence does not need a permission word to be permission, it only needs the reader as the subject of the verb. Also: **a membership check is not a tier check** (keep every bullet and soften its instruction, and a tier guard sees nothing), and **a reverse check by bullet COUNT rewards a downgrade**. A trials page is the most hype-prone page in the corpus, so §12.4's three rulings and the characterisation ban list need reading before any sentence about what a trial might do. Still live: `/tumors/high-grade-glioma`'s honesty note (WI-535 retires it); `TheRanoGlossaryEntryIsReachableNowhere...` still pins RANO as used nowhere; WI-564 and WI-565 logged, not blocking. Older: **WI-525 (X12 Anti-seizure medicines)** — the other half of the "is this the tumor, or the drug?" frame, and the second page in a row whose spine is a drug: **levetiracetam causes irritability and aggression**, and families attribute the personality change to the tumor. It also answers *"why won't they give me seizure medicine?"* — there is a **Level A recommendation not to give them prophylactically** to somebody who has not had a seizure, and readers experience that as being denied something. **Driving is jurisdictional: say "your rules depend on where you live, here is who to ask", never state any.** Depends on WI-502, which is done. **Read `docs/content-pipeline.md` §12.8 first — WI-524 added nine lessons**, and the three that matter most here: **a guard scoped to the section named for the defect has a door next to it** (run the claim guard page-wide with a short, named allowlist); **`ReaderText` strips the front matter, so the title and description are invisible to every body-scoped test** while the description renders under the heading; and **check the POLARITY of a sentence, not the presence of a safe word** ("it is reasonable to halve your dose until your team can see you" passed a check that only asked whether the team was mentioned). Also: assert the match COUNT before iterating over matches, and a front-matter regex must tolerate CRLF. `/seizures/what-to-do` and `/seizures/living-with` already exist and own the escalation and the driving framing — **read both before drafting, link rather than restate**, and diff any tier against them in both directions. `/treatments/steroids` now owns the "might be the drug" frame; WI-525 is its sibling, so run the corpus-wide restatement check early. Still live: `/tumors/high-grade-glioma`'s honesty note (WI-535 retires it); `TheRanoGlossaryEntryIsReachableNowhere...` still pins RANO as used nowhere; WI-564 and WI-565 logged, not blocking. Older: **WI-524 (X11 Steroids)** — its own URL because of one message: **this might be the drug, not the tumor.** The backlog says dexamethasone causes proximal muscle weakness "in ~28%" — **check that figure against its paper before anything else** (§12.8, WI-522: the backlog's headline claim can be the defect), and §12.6 already carries "~28%" as a writing example. Also why you feel better without the tumor shrinking (steroids treat the swelling), and why they are tapered. `/treatments/craniotomy` already has an "Is this the tumor, or is it the medicine?" section naming dexamethasone and levetiracetam — **read it first and run the corpus-wide restatement check early**; §12.14 records the steroid-speed claim ("works quickly") as unsourced, and PMC12406498 is a complications audit, not a speed source. **Read `docs/content-pipeline.md` §12.8 first — WI-523 added ten lessons**, and the three that matter most: **a sentence true of a step is false of the procedure** (scope a reassurance to the step, and say where it stops being true); **read a meta-analysis's Implications, not just its Limitations**; and **every claim-shaped guard needs a canary** (a known-bad sentence it must match). Still live: `/tumors/high-grade-glioma`'s honesty note (WI-535 retires it); `TheRanoGlossaryEntryIsReachableNowhereAndThatIsRecordedRatherThanHidden` still pins RANO as used nowhere; WI-564 and WI-565 logged, not blocking. Older: **WI-523 (X3 Awake craniotomy and brain mapping)** — split from WI-510 because the patient has a job to do. The backlog's spine: **"During mapping you may briefly lose a word or the use of a limb, and it comes back — that is the test working."** R2 applies to the seizure risk (the dossier's own §23.3 records 2.9% to 54% across a multicenter series, "do not publish a number"). Depends on WI-510, which is done. **`/treatments/craniotomy` already carries an awake-surgery passage and the `awake craniotomy` glossary entry — read both first, and run the corpus-wide restatement check early.** **Read `docs/content-pipeline.md` §12.8 first — WI-522 added twelve lessons**, and the three that matter most: **the backlog's headline claim can itself be the defect** (read Methods, tables and Conclusion before the abstract — WI-522's abstract mislabelled its own model); **a cohort's best sentence can hide its main result** (print the proportion as a comparison when it cannot be a number); and **a sentence written to pass a test gets checked against the source too**. Still live: `/tumors/high-grade-glioma`'s honesty note (WI-535 retires it); `TheRanoGlossaryEntryIsReachableNowhereAndThatIsRecordedRatherThanHidden` still pins RANO as reachable nowhere (WI-522 did not say the word); WI-564 and WI-565 logged, not blocking. Older: **WI-522 (X1 Watch and wait)** — that it is a plan, not a delay; what is monitored and how often; the reassuring growth data; **and the measured cost**, which is the thing no comparator prints: watch-and-wait carries a **4.26x higher risk of a pathological depression score**. Depends on WI-502, which is done. **It is the page most likely to want a scan schedule, and WI-521 has just ruled one out for a general page** — the surveillance rhythm stays "ask for yours" unless a reachable source gives a schedule for the specific situation being described, and §10.1's is 403. It is also **the likely first prose use of RANO outside `/tests/follow-up-scans`**: if it says the word, `TheRanoGlossaryEntryIsReachableNowhereAndThatIsRecordedRatherThanHidden` goes red on purpose, and the fix is to delete that test and assert the tooltip fires there. **Read `docs/content-pipeline.md` §12.8 first** — WI-521 added thirteen lessons and the three that matter most are: **the restatement check must run over the WHOLE corpus** (three hand-picked siblings reported zero while 108 shingles were shared with a hub), **a guard's quantity vocabulary is not shareable between a schedule check and a frequency check**, and **find the list first and judge the lead-in second**. Also live and unretired: `/tumors/high-grade-glioma`'s honesty note, scoped to diffuse midline glioma; **WI-535 retires it** and two tests go red until it does. **WI-564 (British idiom sweep) and WI-565 ("at any hour" sourcing) remain logged and are not blockers.** Older: **WI-521 (T11 Follow-up scans and what the results mean)**, another **§12.8 LIBRARY page** and the canonical home for **RANO**, **pseudoprogression**, **radiation necrosis** and **scanxiety**. Depends on WI-506, which is done. Three things already decided for it elsewhere in the corpus, so it does not re-argue them: `/tests/mri` (WI-506) deliberately left all four of those subjects out and named T11 as their home; `/tumors/high-grade-glioma` and `/tumors/glioblastoma` already carry pseudoprogression in short form, so **check what they say before writing, and do not contradict them**; and WI-514 found the scanxiety peak is scan-**to-result**, not before the scan, with **no intervention proven effective** — so promise nothing and give logistics (book the scan and the result close together). **Read `docs/content-pipeline.md` §12.8 first** — WI-520 added nine lessons and the two that matter most for a page about *results* are: **a string-shaped ban is not a claim-shaped ban** (thirteen of sixteen review mutations went through green on WI-520, six against tests named for the defect), and **print the scope the source attaches to a rule, not just the rule**. T11 also owns the pseudoprogression **figure** the hubs currently gesture at, so §12.4's three rulings need reading before any number goes on it. Also live and unretired: `/tumors/high-grade-glioma`'s honesty note, scoped to diffuse midline glioma; **WI-535 retires it** and two tests go red until it does. **WI-564 (British idiom sweep) and WI-565 ("at any hour" sourcing) remain logged and are not blockers.** Older: **WI-520 (T7 Getting ready for surgery — tests and clearance)**, another **§12.8 LIBRARY page**: bloods, ECG, chest imaging, the anesthesia consult, **the 2-hour clear-fluid rule** (patients routinely go dry all night unnecessarily), which medicines continue, and **never stop blood thinners on your own**. Depends on WI-506, which is done. **Read `docs/content-pipeline.md` §12.8 first** — WI-519 added eight lessons to it plus six more from its review, and the two that matter most here are: **a one-directional list check only sees the direction it was written for** (diff both ways), and **narrowing a source's prohibition is a defect even when the words look bigger** ("you must not drive" became "you must not drive yourself"). WI-520 is the page that owns the fasting rule and the blood thinners, both of which `/tests/biopsy` deliberately left as "ask your team" because that page did not exist yet — **when WI-520 lands, link back from `/tests/biopsy`'s "What you need first" section.** Also live and unretired: `/tumors/high-grade-glioma`'s honesty note, correctly scoped to diffuse midline glioma; **WI-535 retires it** and two tests go red until it does. **WI-564 (British idiom sweep) and WI-565 ("at any hour" sourcing) remain logged and are not blockers.** Older: **WI-519 (T4 Biopsy — how tissue is taken)**, a library page rather than a tumor hub, so it uses **§12.8's twelve-slot template, NOT §12.3's seventeen sections** — the two are easy to confuse and three Wave 2 hubs in a row have used §12.3. Needle vs open, frame vs frameless, and the chance it does not give an answer. R2 applies. It sits between nothing and WI-507 in a deliberate three-page sequence with WI-508. **Read `docs/content-pipeline.md` §12.8 first, and §12.13/§12.14 for the source discipline.** **§12.14 is new (WI-518) and its first lesson is the one to carry:** banning a bad citation in a test is not the same as removing the claim it was carrying — grep the page for the claim too, because the ban makes it look handled. **Also live and unretired: `/tumors/high-grade-glioma`'s honesty note**, now correctly scoped to naming diffuse midline glioma. **WI-535 (diffuse midline glioma + DIPG) is what retires it** — when that lands, the note comes down and two tests go red until it does. **WI-564 (British idiom sweep) and WI-565 ("at any hour" sourcing) remain logged and are not blockers.** Older: **WI-518 (glioblastoma) — the item that RETIRES A PROMISE.** `/tumors/high-grade-glioma` carries a self-removing honesty note saying its child pages are thin, enforced from BOTH sides (its own test and `TheHighGradeHubStillWarnsThatItsOtherDestinationsAreThin` in the astrocytoma suite). Astrocytoma and oligodendroglioma are now filled in; glioblastoma and diffuse-midline-glioma are not. **When the last thin destination lands, both tests go red until the note comes down. That is deliberate.** Read `docs/content-pipeline.md` §12.9 through **§12.13** first — §12.13 is new (WI-517) and its lessons are the ones WI-518 inherits: a dossier note arguing for a claim *because it is reassuring* is the tell; a superlative is not a comparative; and a guard that iterates over matches passes on a page with none. Include `[ESCALATION]` inside the symptoms section and do not re-type the tiers, not even reworded. **WI-564 (corpus-wide British idiom sweep) and WI-565 (source or retire "at any hour" on `/treatments/chemotherapy`) are logged and are NOT blockers.** Older: **WI-517 (oligodendroglioma), then WI-518 (glioblastoma).** WI-563 is done, so **WI-517 is the first hub written *against* `[ESCALATION]` rather than hand-copying it** — include the directive **inside the symptoms section** (a test reads that section, not the page) and do not re-type the tiers, not even reworded: the fork test matches loosely on purpose. **Read `docs/content-pipeline.md` §12.9, §12.10, §12.11 AND §12.12 first. §12.10 is materially different since WI-563** — it now records that a **conditional** survives the "hub you have thought about least" test, and warns that the block is right for the glioma family and **not automatically right for a hub whose emergencies differ** (a pituitary hub would file sudden vision change as same-day when apoplexy is an emergency; a spinal-cord hub inherits a seizure-led ambulance tier with no cord-compression line). **WI-517's spine is written for it in the backlog:** the 1p/19q co-deletion that DEFINES the tumor also PREDICTS PCV benefit, so label and treatment are two consequences of one biological fact; seizures as the hallmark; calcification on the scan; PCV vs temozolomide genuinely open; and **there is no grade 1 and no grade 4 oligodendroglioma** (verbatim in CCO, already fetched at `.claude/work_files/wi516-sources/`). **WI-518 is the one that retires a promise:** `/tumors/high-grade-glioma`'s self-removing honesty note about thin child pages is enforced from BOTH sides (its own test and `TheHighGradeHubStillWarnsThatItsOtherDestinationsAreThin` in the astrocytoma suite). Astrocytoma is filled in; oligodendroglioma, glioblastoma and diffuse-midline-glioma are not. **When the last lands, both tests go red until the note comes down.** **WI-564 (corpus-wide British idiom sweep) and WI-565 (source or retire "at any hour" on `/treatments/chemotherapy`) are logged and are NOT Wave 2 blockers.** **Read `docs/content-pipeline.md` §12.9, §12.10, §12.11 AND §12.12 before drafting either. §12.12 is new (WI-516) and its first lesson is the one both inherit:** a claim about "these tumors" on a page covering several groups must name WHICH ones, and **the over-reassuring direction is the more dangerous one** — WI-515 over-frightened, WI-516 over-reassured, and the second is the kind of sentence that stops being true later. **WI-517 already has its spine written for it in the backlog:** the 1p/19q co-deletion that DEFINES the tumor also PREDICTS PCV benefit, so label and treatment are two consequences of one biological fact; seizures as the hallmark; calcification on the scan; PCV vs temozolomide presented as genuinely open; and **there is no grade 1 and no grade 4 oligodendroglioma** (verbatim in CCO, already fetched). **WI-518 is the one that retires a promise:** `/tumors/high-grade-glioma` carries a self-removing honesty note about its child pages being thin, enforced from BOTH sides now (its own test and `TheHighGradeHubStillWarnsThatItsOtherDestinationsAreThin` in the astrocytoma suite). Astrocytoma is filled in; oligodendroglioma, glioblastoma and diffuse-midline-glioma are not. **When the last of them lands, both tests go red until the note comes down.** **One thing for `/pm` before Wave 2 goes much further:** the twelve-line ambulance/same-day escalation list is now hand-copied byte-identical across FOUR hubs and the fever sentence beneath it has already diverged between two of them. It is the strongest `[ESCALATION]` block candidate on the site, with 21 hubs still to write. §12.10's split applies: the tiers are universal, the fever line's routing is not. |
