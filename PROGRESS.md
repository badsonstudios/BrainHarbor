...
| **In progress** | **WI-543 (Spinal cord tumor, deepened) — ALL GATES PASSED 2026-09-19, awaiting commit + PR**, branch `feature/wi-543-spinal-cord-tumor` (from develop at `8d681d3`). **The strength split WI-536 handed over is SETTLED, and the answer was that it was never one claim.** Two claims wore one sentence: metastatic cord compression keeps an emergency-room rule (ACS, OncoLink), while a PRIMARY tumor in or pressing on the cord gets a same-day call, because no fetched patient-facing source applies an emergency rule to a named primary cord tumor outside cauda equina (AANS, ABTA, Columbia, NCI PDQ all list the signs and give no urgency rule — absences recorded in a 40-source table scored BEFORE drafting). ACS grades the two situations differently on its own two pages. A carve-out applies to BOTH and is stated as more urgent, not less: new bladder/bowel trouble, saddle numbness, or weakness worsening over hours to days is the emergency room now. `/tumors/meningioma` re-tiered and given the carve-out; `/tumors/brain-metastases` keeps its own tier (its population is metastatic) and gains the route to this page it never named. `SpinalCordBlockTests.TheStrengthSplitWithTheSiblingHubsIsPinnedUntilWi543SettlesIt` was **rewritten, not deleted**. WI-528's "three pages, one wording" was retired deliberately, with reasons. Blocks 5 in / 3 out; `[SPINAL-CORD]` EXCLUDED despite the name match. Taxonomy rule (WI-412) untouched and handproofed. **Gates:** suite 2,313/2,313; ContentCheck 273/0 at grade 5.8 (meningioma 5.6, brain-metastases 5.7); **53 break mutations red on LF AND CRLF** (3 survivors — two dead guards, one weak mutation — each fix re-run against the mutation that exposed it); **6 render guards handproofed**, including the EXCLUDED block composed back in; six rendered reads; privacy scan 0 findings over 2,425 net-new lines; five `/review` rounds (in rounds 2, 3 and 4 the blocker was created by the previous round's fix). **REMAINING: commit → PR into develop → release PR into main → deploy smoke.** `wi543/smoke.py` still refuses until CHECKS/CONTROLS/baseline are retargeted. |
