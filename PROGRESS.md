...
| **In progress** | **Nothing in progress.** WI-548 (Sweep: every taxonomy type has a full guide) is COMPLETE and awaiting merge (branch `feature/wi-548-template-sweep`). Next item is **WI-549 (T8 Neurological and thinking/memory testing)**, not yet started. |
| **WI-548 (this item)** | **WI-548 IS COMPLETE, NOT YET MERGED.** Branch `feature/wi-548-template-sweep` from develop at `1ba66c1`. **Acceptance met by `TumorHubTemplateSweepTests`, which iterates TAXONOMY SLUGS rather than directory files** (`CaregiverSectionTests.TumorHubs()` enumerates files and cannot see a slug whose page is missing). Counts re-verified rather than trusted: 23 slugs, 23 files, one per slug. Both named guards in `TumorsPageTests` untouched. **GATE 1 RULING, WRITTEN BEFORE IMPLEMENTING** (now content-pipeline §12.15): the unit of obligation is a QUESTION, not a heading string; a page answers it under the standard heading, answers it under its OWN heading, or ROUTES it (§12.10), and may NEVER simply drop it — which is what stops the exception list being "the pages that currently fail". **Four classes keyed to a property of the SLUG: Hub (DEFAULT, all 17), Secondary (`brain-metastases`), Axis (`pediatric-brain-tumor`), Umbrella (`all-brain-tumors`).** A new entry is a Hub; the three exceptions are PINNED IN THE TEST, so an exemption can only be acquired by editing a test in review. `spinal-cord-tumor` looked like the strongest exemption case and needed none — a FRAMING ruling is not a TEMPLATE ruling. **THE RULING REFUSED ONE OF ITS OWN CANDIDATE EXEMPTIONS: `/tumors/pediatric-brain-tumor` said NOTHING about the tumor coming back** — no section, no sentence, no route. The class was not widened to fit the page; **the page was written to fit the class**. **AND A SHIPPED GUARD OVERRULED ME MID-ITEM:** the new section's best source was Cancer Research UK, which that page is BARRED from (WI-537, re-checked twice). `TheFrontMatterCarriesTheContractAndNoBarredSource` went red. The ban was HONOURED, not overturned for one convenient quote — rebuilt on St. Jude, ACS and Dana-Farber, each re-fetched and read live (§12.8: a dossier cannot support a published claim). The rebuild IMPROVED the content: the vocabulary is now relapse / recurrence / progression, the split the research pass itself named as the corpus gap, with the general-cancer source SCOPED on the page. The same page's British-forms guard then caught "fits" in "ask which word fits your child"; reworded, guard untouched. **FIVE ABSENCES RECORDED WITH SOURCES CITED**: no repeat biopsy, no confirmatory rescan, no tumor board, no sourced claim about how relapse feels, no frequency for trials. **PROOF: 36 break mutations red on LF AND CRLF (72/72), no survivors and no ambiguous anchors on the first run; the plain suite 2,469/2,469 on a fully LF corpus AND a fully CRLF corpus** (WI-547's lesson discharged directly — the harness proves mutations are CAUGHT, not that the unmutated corpus is GREEN); **3 handproof cases** against the one render guard, one per claim; ContentCheck 277/0; privacy scan **0 findings over 1,694 net-new lines in 9 files**; **FIVE `/review` rounds (5 blockers, 2, 2, none, none)**. **Rounds 2, 3 and 5 each found their finding INSIDE the previous round's fix**, including a "derived" canary that was STRICTLY WEAKER than the typed floor it replaced, and the umbrella page silently dropping §12.3 question 2 inside the very test that forbids dropping questions — visible for a whole round as a stale number (comments said eleven, the array held ten). **An incident worth keeping: a killed break harness left a mutation on `brain-metastases.md` reading as ordinary prose.** The marker file flagged it; my FIRST integrity check was itself wrong (it compared the backups against their own checksums, not against the live files) and would have reported all-clear. **For `/pm`:** `/tumors/pediatric-brain-tumor` now carries NO sourced sentence about how relapse feels, because the best writing on it is on a publisher that page is barred from — a real gap, recorded not resolved. `HeadingsOfSource` does not strip YAML front matter, so a `##`-looking line in a comment would count as a heading a page owes (no page has one today, all 23 checked). Johns Hopkins and ABTA blocked for the fourth item running. Next: commit, PR into develop, release PR into main, **pre-merge smoke REQUIRED** (this item changes two reader-facing pages). |
