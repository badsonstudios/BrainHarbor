...
| **Next up** | **WI-567 - `/where-your-tumor-is`, the location page.** Depends on WI-568, which is now done: **the region list is settled at NINE** and the block already carries all nine, so WI-567's acceptance line about "the six in `[MECHANISM]` plus three" has been CORRECTED in the backlog - do not write to the old one. **It OWES A DOOR BACK to `blocks/mechanism.md`**: `MechanismBlockTests.TheRouteToTheLocationPageIsOwedAndNotYetWritten` goes RED the day the page ships, and whoever ships it adds the route and deletes that test. **Verify `moffitt.org/cancers/brain-tumor/location/` by hand BEFORE writing** - it 403'd during research and is the one unverified hole in the "no comparator does this" conclusion. **Carry forward for `/pm`, NEW from WI-568:** **WI-574** (block source titles rendering on all including pages - third recording, zero fixes); the `/tumors/meningioma` location duplication handed to **WI-569** and pinned by a test; `/tumors/atrt:222` says "See the mechanism note below" and no such note exists on that page. **Still open:** WI-554 (Rehabilitation) owes a door back to `/tests/neuro-exam-and-memory-testing`; WI-564 (idiom sweep) should PLANT idioms rather than only grep for them; Johns Hopkins, ABTA and Cancer Research UK remain barred on specific pages - check per page, not per domain. |
