...
| **In progress** | **WI-545 (Chordoma)** — started 2026-09-20 on `feature/wi-545-chordoma`. Step: **all work complete and green; awaiting Gate 2 (commit approval).** Nothing is committed yet. All green: suite **2,378/2,378**, ContentCheck **275/0** at grade **5.9** (proton-therapy 5.4), all three restatement guards **0 collisions** (exit 0, verified unpiped), normalisation 0 offenders, **four rendered reads**, break harness **142/142 red on LF and CRLF**, handproof **8/8**, privacy scan **0 findings over 2,457 net-new lines**. Blocks 5 in / 3 out. **12 sources total were re-fetched and recorded with verbatims** across the two rounds. Round 1 blockers: (1) the emergency-scoping sentence scoped by PUBLISHER and the page then quoted Cleveland Clinic doing exactly that — reworded to scope by SUBJECT, with two bans + canaries; (2) load-bearing claims sourced in the research pack but NOT recorded here (the WI-544 shape). Round 2 blocker: three MORE claims of the same class, incl. one the round-1 edits had touched without re-sourcing. **Two review findings were REJECTED on evidence**, both recorded with verbatims: the ">half recur" sentence is the Chordoma Foundation's own wording about ALL chordoma tumors; and the caregiver comparative is the Foundation's own survey finding, not a cross-measure comparison. Six claims were REWORDED because sources did not support the wording (pain quantifier, lungs, fatigue comparative, "harmless", metastasis, chemo-fever scope). §12.8 lessons, backlog checkbox and this log entry are written. Still to do, all after Gate 2: commit, PR into `develop`, release PR into `main`, **run `wi545/smoke.py` BEFORE the release merge and require it to FAIL** (the hub 404s in production today, so it cannot pass by accident), then the post-deploy smoke. A NEW §12.3 hub, no stub existed. Prior item WI-544 (ATRT) shipped 2026-09-19 and is CLOSED — do not reopen. |
| **WI-544 (previous)** | **WI-544 is SHIPPED AND LIVE** (PR [#155](https://github.com/badsonstudios/BrainHarbor/pull/155) → `develop` (build-test pass 3m34s), release PR [#156](https://github.com/badsonstudios/BrainHarbor/pull/156) → `main`, merge `5f085ed`, build-test success 4m10s → deploy success 1m10s, **first attempt — the sixth release in a row to deploy first time**). **Post-deploy smoke clean:** `controls ok (2)` first, then `/tumors/atrt` 200 at **56,965 bytes**, past its measured 50,000 floor; all 21 positives present, all 14 absences absent; `/tumors` grew 17,508 → 17,590 as the index picked up the new page, every other route byte-identical to baseline. The SAME probe had been required to FAIL before the merge and did — controls ok, `/tumors/atrt` 404, exit 1 — so `smoke clean` here is evidence rather than an untested instrument. **WI-545 (Chordoma) is next.** |
...
- **2026-09-20** — **WI-545 done** (`/next-item`): `/tumors/chordoma`, a NEW §12.3 hub
