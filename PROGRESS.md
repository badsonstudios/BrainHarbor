...
| **In progress** | **WI-546 (CNS germ cell tumor)** — started 2026-09-21 on `feature/wi-546-cns-germ-cell-tumor`. Step: **all work complete and green; awaiting Gate 2 (commit approval).** Nothing is committed yet. All green: suite **2,416/2,416** (+37 guards, +1 spinal-cord includer register entry), ContentCheck **276/0** at grade **5.6**, both restatement variants **0 collisions**, normalisation clean, **break harness 240/240 red on LF and CRLF, first run** (120 mutations), **handproof 10/10 across all 7 render guards**, privacy scan clean, two full rendered reads. Blocks 6 in / 2 out. **37 sources** recorded with verbatims. **Three /review rounds**: round 1 BLOCKER — the page said these tumors "are not usually given a grade" because the WHO grade table omits them, but that table is "grades of selected types" and omits graded tumors too, and the same paper says a germinoma "can be assigned" grade 4; the page now makes no claim either way. Rounds 2 and 3: no blockers (round 2 caught the round-1 markers fix overshooting its source). Gate 1 decisions A/B/C applied; WI-552 now carries the obligation to add doors from this page and ATRT. §12.8 lessons, backlog checkbox and this log entry are written. Still to do, all after Gate 2: commit, PR into `develop`, release PR into `main`, **run `wi546/smoke.py` BEFORE the release merge and require it to FAIL** (the hub 404s in production today), then the post-deploy smoke. Prior item WI-545 (Chordoma) shipped 2026-09-20 and is CLOSED — do not reopen. |
| **WI-545 (previous)** | **WI-545 is SHIPPED AND LIVE** (PR [#157](https://github.com/badsonstudios/BrainHarbor/pull/157) → `develop` (build-test pass 3m47s), release PR [#158](https://github.com/badsonstudios/BrainHarbor/pull/158) → `main`, merge `8215b43`, build-test success → deploy success, **first attempt — the seventh release in a row to deploy first time**). **Post-deploy smoke clean:** `controls ok (2)` first, then `/tumors/chordoma` 200 at **60,105 bytes**, past its measured 55,000 floor; all 15 positives present, all 16 absences absent; `/tumors` grew 17,598 → 17,658 as the index picked up the new page, every other route byte-identical to the pre-deploy baseline. The SAME probe had been required to FAIL before the release merge and did — controls ok, `/tumors/chordoma` 404, exit 1 — so `smoke clean` here is evidence rather than an untested instrument. A NEW §12.3 hub for a tumor that is mostly NOT in the brain: blocks 5 in / 3 out with `[MECHANISM]`, `[SPINAL-CORD]` and `[POSTERIOR-FOSSA-SYNDROME]` all EXCLUDED. Suite **2,378/2,378** (+33 guards), ContentCheck **275/0** at grade **5.9**, break harness **142/142 red on LF and CRLF**, handproof **8 cases / 7 render guards**, privacy **0 findings over 2,457 net-new lines**. **WI-546 (CNS germ cell tumor) is next.** |
...
- **2026-09-21** — **WI-546 done** (`/next-item`): `/tumors/cns-germ-cell-tumor`, a NEW §12.3 hub
