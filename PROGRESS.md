...
| **Next up** | **WI-541 — Acoustic neuroma, deepened.** Depends on: WI-513. NF2-related schwannomatosis, renamed 2022; links forward to WI-551 (hearing) and WI-531 (SRS). **CORRECTED ON CHECKING: `/treatments/stereotactic-radiosurgery` ALREADY EXISTS and is linked from `/tumors/meningioma`, so SRS is ROUTED, not owned** — the "neither is built" note carried forward into this row was false, and a note claiming a page is unbuilt would send the next editor to write one that ships today. **WI-551 (hearing) IS genuinely unbuilt** — there is no hearing or vision test page — so the hearing-test material is owned by nobody and this page writes a proportionate slice worded to stand alone, the shape `/tumors/pituitary-tumor` used for the same two gaps. **READ BOTH `/tumors/pituitary-tumor` AND `/tumors/craniopharyngioma` BEFORE DRAFTING, AND TREAT THEM AS HAZARDS AS MUCH AS MODELS.** This is the third consecutive benign, locally-pushy tumor outside the brain substance whose page turns on a nerve and a rename — close enough to copy by accident. The restatement guard is **BIDIRECTIONAL, and craniopharyngioma ships with NO ALLOWLIST AT ALL**: borrowing its prose turns THAT page red, and an allowlist on the new page cannot fix it. Take the stance, write your own sentences. `/treatments/steroids` is now read by **two** hubs' guards, so any adrenal-crisis wording must read at one strength across three pages. **DECIDE FROM SOURCES WHETHER THIS TUMOR OWNS AN EMERGENCY TIER AT ALL** — WI-539 had two and WI-540 reached the OPPOSITE answer on apoplexy from evidence that looked the same; the deciding facts were rarity wording repeated in a paper's introduction AND conclusion, plus two verified SILENCES in the general reference chapters. **Tools in `.claude/work_files/wi540/`, to copy into a NEW `wi541/` so "no allowlist" stays a property of the directory:** `all-collisions.py` (imports `shingles.py`'s own functions so it cannot drift, and prints the corpus intersection **UNCAPPED**), `smoke.py` (falsifiable — byte baseline, control fragments, and NEGATIVE probes for rulings that are absences), plus dryrun/harness/handproof with asserted imports and `uniq()` refusing ambiguous anchors (154 mutations, zero ambiguous, green on LF and CRLF first run). **What WI-540 learned that the next item will hit:** (a) **A CAPPED REPORT IS NEVER AN INVENTORY** — the suite guard and the replica both `.Take(4)` per file and order their sets differently, so six passes reported "fresh" collisions that had been under the cap all along; run the uncapped tool once. (b) **PROVE THE INSTRUMENT CAN SAY YES BEFORE ACCEPTING IT SAYING NO** — every zero here was believed only after the same tool was made to report non-zero. (c) **A WEAK MUTATION IS NOT A WEAK GUARD** — the handproof's one survivor came from writing the mutation out of a test COMMENT instead of the mechanism, and the comment was wrong (the tumor index depends on `taxonomy.yml`, not on the page's description). (d) **A GUARD THAT REPORTS INSTEAD OF ENFORCING IS NOT A GUARD** — `|| echo "refusing"` prints a refusal and proceeds; git's own `-d` is what actually protected the branch. (e) **EIGHT REFERENCE DEFECTS, FOUR CREATED BY FIXES**, one reproducing the defect it was repairing in the same sentence: read each edited sentence together with the one BEFORE it, resolving every pronoun to the NEAREST noun — and note a DELETION strands pronouns too. (f) **CLEARING THE RESTATEMENT GUARD IS ITSELF THE MECHANISM THAT HIDES BORROWED REASONING** — the item's only blocker was a sibling's sentence reworded until the shingle check went quiet, carrying its reasoning intact onto a page whose own ruling contradicted it. Re-derive the CLAIM, not the wording. (g) **A SUMMARY OF A SAFETY RULE IS A RESTATEMENT OF IT** — the caregiver section silently narrowed both tiers and nothing saw it, because every tier guard reads the symptoms section. (h) **TEST COMMENTS CITED THREE SOURCES THE PAGE DOES NOT CARRY** — the claims were all carried; the rationale was not. (i) **A PIPE HIDES THE EXIT CODE AND DESTROYS EVIDENCE** — `dotnet test | tail` reported exit 0 on a five-failure run and lost two failure names permanently. **Still live from earlier items:** (j) the shunt clause reaches a composed reader **five times**, a ceiling rather than a target. (k) `[SPINAL-CORD]` has four includers and `[POSTERIOR-FOSSA-SYNDROME]` three, asserted exactly, so a new includer fails until registered. (l) **WI-566 is open**: `ReaderText` silently eats three characters on six live guards across four pages. (m) `/tumors/pediatric-brain-tumor` still owes a reciprocal door back to `/tumors/craniopharyngioma`. (n) "Sight test" is British register and belongs to the follow-up idiom sweep, not to a tumor item. **Carry forward from WI-538, still live:** (1) the shunt clause now reaches a COMPOSED reader **five times**, each on a distinct trigger and none redundant — a ceiling rather than a target, so a sixth needs a reason. (2) `[SPINAL-CORD]` has **four** includers and `[POSTERIOR-FOSSA-SYNDROME]` **three**, both asserted exactly, so a new includer fails until registered. (3) **Three tools are saved rather than re-derived:** `privacy-scan.py` (enumerates UNTRACKED files — the throwaway version did not, and reported "clean" on roughly half a change), `shingles.py` with `allowlist.txt` (offline replica of the corpus restatement guard — copy allowlist entries from the TEST, and validate against a known-clean run before trusting it, which this one FAILED first time by composing a page the guard reads raw), and `anchor-audit.py` (finds mutations that bite front matter instead of the body). (4) **WI-566 is open**: `ReaderText` silently eats three characters on six live guards across four pages — a test-helper defect, deliberately NOT filed under the British-idiom sweep. (5) §12.8 now carries the corrected all-zero-probe rule — control fragments separate a broken probe from a failed deploy. (6) The habit that cost this item most: **five false negatives from four mechanisms** — hard wrapping twice, a comma where whitespace was assumed, and searching for only PART of a sentence being changed. **Normalise the way the guard normalises.** **Carry forward from WI-539:** (7) **A GUARD OVER A PARAGRAPH CANNOT SEE THE SCOPE OF ITS LEAD-IN** — seven signs were asserted one by one while the tier's ENTRY was a conjunction excluding the commonest presentation. When a rule has an entry condition, assert the entry, not only the contents. (8) **WHEN A SENTENCE MAKES A CLAIM ABOUT A LIST, THE GUARD MUST OPEN THE LIST** — "one item on the list below is different for you" was false about its own list and survived two review rounds, because every guard on both pages read the CONDITIONAL and none read the LIST. (9) **A FRONT-MATTER CORRECTION MUST NOT QUOTE WHAT IT CORRECTS** — a raw-file guard cannot tell a quotation from the claim, and this bit three times in one item, twice while writing a note whose subject was the trap. (10) **A REVIEWER'S "VERBATIM" IS A CLAIM LIKE ANY OTHER** — round 3 cited a quotation that is not in the chapter; agreeing with the conclusion is exactly when you stop checking the evidence. (11) **A DOSSIER IS NOT A SOURCE**: `docs/research/tumor-guides/*` cannot support a published claim, and reaching for a project file instead of a source file is the tell. (12) Tools saved in `.claude/work_files/wi539/`: `uniq()` (refuses an ambiguous anchor rather than silently taking the first — 111 mutations, zero ambiguous), plus dryrun/harness/handproof with asserted imports so a copied script cannot report success over another item's pages. |
