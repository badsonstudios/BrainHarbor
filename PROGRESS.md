...
| **Next up** | **WI-536 — Ependymoma, deepened** (Wave 4; depends on WI-514, done; see backlog.md). Nothing scouted. **Carry forward:** (1) the `/tumors/ependymoma` stub says its tumor "can block the flow of fluid" and includes NEITHER [MECHANISM] NOR [ESCALATION] — add both together (`EscalationBlockTests` checks COMPOSED hubs; the block now has three conditionals: chemo fever, post-surgery fever, shunt). (2) **Ependymoma also grows in the spinal cord**: WI-535 put a PAGE-LOCAL right-away rule for a tumor in the cord after [ESCALATION] on `/tumors/diffuse-midline-glioma` (CRUK, attributed; see its front matter). A second hub needing it is the §12.10 moment to decide whether it becomes a block conditional instead of a second copy. (3) **Every same-day line a hub writes itself must say "right away if there is a shunt"** (WI-535 round-2 blocker); DMG/DIPG tests `EverySameDayLineOnThePageHonoursTheShuntRule` are the pattern. (4) Sentence-scanning guards must strip markdown heading lines before `SentencesOf` (it merges a heading into the next sentence). (5) Curability goes in the grade section at the sibling hubs' strength, never softened behind the gate. (6) Harness: copy `break-tests.py`, `dryrun.py`, `probe.py`, `handproof.py` from `.claude/work_files/wi535/`, repoint `sys.path` in the first three and PAGE/CLASS names in handproof; fix scripts that check every anchor before writing are in the same folder. |
