...
| **In progress** | **WI-537 IS LIVE.** PR **#141** into `develop` (build-test pass 3m50s, squash-merged, tip `b05bd03`), release PR **#142** into `main` (build-test pass 3m42s), and the merge commit's `main` run went **build-test success → deploy success first time** (`538abd9`). **Live smoke 9/9 routes 200:** `/`, `/research`, `/get-help-now`, `/glossary`, `/tumors/medulloblastoma`, `/tumors/ependymoma`, `/treatments/chemotherapy`, `/tumors/pediatric-brain-tumor`, `/tumors/glioma`. **A PRE-DEPLOY BASELINE made the smoke falsifiable instead of merely reassuring, and it is worth repeating:** before the merge `/tumors/medulloblastoma` served **10,753 bytes with ZERO hits** for "posterior fossa syndrome", "craniospinal", "group 3" and "Non-WNT"; after it, **51,670 bytes with all of them**. Ependymoma 51,748 → 52,160, chemotherapy 38,916 → 39,567, the pediatric hub 11,021 → 11,059, and `/` plus `/get-help-now` byte-identical — so the deploy provably changed exactly what it should and nothing else. **Content probes on the live pages:** the grade sentence ("Every medulloblastoma is grade 4 whatever its kind"), "Both names are still used", the spread-to-the-spine tier clause on BOTH hubs, the composed block ("Most children slowly get better", the restored "usually starts in hospital" hedge, the ambulance carve-out), **`def-posterior-fossa-syndrome` present and `def-posterior-fossa"` ABSENT** (the tooltip decision holding in production, which was the defect only a rendered read caught), the chemotherapy door's "usually includes"/"some plans add lomustine" with "most often cisplatin" gone, and **no unresolved directive, fence or marker anywhere**. **SMOKE-PROBE LESSON FOR THE NEXT ITEM:** probing rendered HTML with a contiguous text match FAILS wherever a glossary tooltip is injected mid-sentence. The drug sentence reported n=0 on the hub while the same string passed on `/treatments/chemotherapy`, purely because `def-vincristine` and `def-lomustine` fire inside it there and had already fired earlier on the other page. Strip tags, normalise whitespace, and probe tooltip-free FRAGMENTS — and treat an all-zero probe run as proof the PROBE is broken, not the page (the first attempt returned 0 for every string, including ones that must exist). The WI-537 pickup history follows. **WI-537 (Medulloblastoma, deepened) — started 2026-09-15**, branch `feature/wi-537-medulloblastoma` (from develop at 0891b5e). **Sources DONE:** 62 files, **225 quotes script-checked** (the verifier now normalises NFKC + curly quotes + dashes + whitespace, and caught 4 real transcription errors), `.claude/work_files/wi537-sources/NOTES.md`. **CODE DRAFTED AND GREEN (2026-09-15): suite 2117/0** (2065 before this item, so +52 tests), **ContentCheck 271/0** (medulloblastoma **5.6**, ependymoma 5.4). `/tumors/medulloblastoma` written as a full §12.3 hub with `[MECHANISM]`+`[ESCALATION]`+`[SPINAL-CORD]`, `[CROSSWALK]`, `[TUMOR-BOARD]`, `[CAUSES]`, `[CAREGIVER]`. **NEW BLOCK `blocks/posterior-fossa-syndrome.md`** factored out of `/tumors/ependymoma` at its second use (§12.8), carrying the ambulance carve-out that is the whole reason it is a block rather than two copies. New test files `MedulloblastomaPageTests.cs` (34 content + 6 render) and `PosteriorFossaSyndromeBlockTests.cs` (10). **The normalisation guard was PROMOTED into `CuratedPage.AssertNoWarningSignIsNormalised`** at its second use, and doing so exposed two holes that were invisible while it ran on a RAW page: `usual` matched the escalation block's own red flag "a headache much worse **than usual**", and a bullet's tier lives in its LEAD-IN paragraph, which a bullet-scoped check cannot see. Both fixed in the shared helper; `/tumors/ependymoma`'s three changed guards now read the COMPOSED page (read raw they went green by losing their subject, §12.10's trap). **Findings fixed along the way:** the draft said WNT "responds best" (a banned characterisation, WI-517); it said "Straight after surgery" (a British idiom §12.10 names); it **restated `/treatments/proton-therapy`'s own sentence** about the beam stopping at the tumor, which turned that page's restatement test red, so the page now routes there and carries only the craniospinal-field reason (new source PMC10051027). **Rendered read 1 done** (`rendered-medulloblastoma.txt`): the tooltip suppression was on the WRONG term. Longest-match does NOT protect the shorter term here, because the page names the syndrome TWICE (the section, and the support list signposting St. Jude), and the tooltip is first-occurrence-only, so the bare `posterior fossa` term fired INSIDE the syndrome's name at the second mention. Now `!%posterior fossa%`, the same decision ependymoma made. Read 1 also fixed a circular WNT bullet, "not himself or herself", and "at the back of the brain" three times in four lines. The block gained PMC12921523 (JAMA Netw Open / St. Jude cohort) for the complication attribution, because read 1 showed the only support for that sentence on this hub was a chapter titled "Ependymoma". Harness scripts repointed and written: `mutations.py` (~130), `handproof.py` (8 render cases incl. both halves of the tooltip decision), `render-read.py`. **NEXT: dryrun, break harness on LF and CRLF, handproof, rendered read 2, `/review` until a round finds no blockers, then docs (§12.8 + backlog) and commit → PR into develop → release PR into main → deploy smoke.** **`/review` round 1 (code-reviewer agent): 2 BLOCKERS, 7 should-fix, 6 nits, plus a substantial "correct as-is" section** confirming the carve-out, the tier placement, the absence of forbidden figures, the PNET handling and the promoted guard. **Every load-bearing quote was re-verified by hand before acting** (WI-536's rule: review quotes fail in BOTH directions). A cautionary note on that: my own first verification script reported the CNS5 "(Groups 3 and 4)" phrase ABSENT and then crashed printing it, because the saved page uses U+2010 and my normaliser only folded U+2013/U+2014. Same class of error §12.8 already records; fold the whole hyphen family. **B1 (confirmed, real): the page claimed posterior fossa syndrome happens "more so than after most other operations there", a comparative NO cited source makes** and which St. Jude contradicts in kind ("also can happen after surgery to remove other kinds of brain tumors in the posterior fossa. Those include astrocytoma and ependymoma"); ACS writes "such as medulloblastoma or ependymoma" with no comparison. Worse, **a test PINNED the sentence** — a new shape of the §12.10 trap, where a guard's subject is a phrase whose problem is its TRUTH. **B2 (confirmed, real): "group 3" and "group 4" are CURRENT CNS5 names, not retired ones** (CNS5 lists "Medulloblastomas, non-WNT/non-SHH (Groups 3 and 4)"; St. Jude writes "Group 3 tumors are the second most common"), and the page contradicted itself, using them in the present tense 35 lines earlier. A parent holding a "Group 4" report was told in the report-reading section that the label was obsolete. Both fixed, both now asserted. **Should-fix taken:** the residual-tumor factor said "bigger than a small coin" when the sourced threshold is an AREA of 1.5 cm2 on the post-op scan, which is SMALLER than a dime's face, so the analogy moved the bar about 70% the REASSURING way and turned an area into a 3-D "piece" (now routes to the measurement, which also splits a 46-word five-condition sentence); **"do well" added to `Characterisations`** — the one inflection the list lacked beside "does better"/"responds well"/"responds best", and said of ONE molecular group outside the gate it ranks every other group by implication, so the sentence now gives WHO's reason ("responsive to current therapeutic regimens") instead of a population claim; the report section now says what the group is FOR (St. Jude: "and to plan treatments"); `ComposedSubsectionLf` promoted to `CuratedPage.ComposedSubsection` (this item promoted one helper and duplicated another in the same diff); the Brain Tumor Network bullet now says "adult patients", which is what its source says, on a page whose reader is usually a parent. **One recommendation REJECTED with reason:** moving the lasting-impairment sentence INTO the block would assert a medulloblastoma cohort's finding of ependymoma readers, which is exactly §12.10's "would this be true on the hub you have thought about least?" failure. Taken instead: the cohort citation is **reverted out of the block and kept page-local**, and a test now pins that it stays out. **Deferred with reasons:** a glossary entry for "embryonal tumor" (the word is on ONE page today and that page defines it inline, so an entry would echo or need suppressing; §12.8's second-use threshold applied to vocabulary), the `blocks/mechanism.md` "feeling sick" idiom (reaches 8+ hubs, WI-536 already weighed and deferred it, needs its own item and review), and deriving `EscalationBlockTests`' hub list from `DirectBlockNames` (pre-existing, nothing unguarded). **A sourcing gap I found while checking the deferral:** the page's "embryonal tumor" gloss traced to nothing in its own front matter, so ACS's children's-types page is now cited for "Embryonal tumors start in early forms of nerve cells in the central nervous system". mutations.py now **155, dryrun 0 problems**, with new breaks for both blockers, the coin analogy, the "do well" ban and the block's re-added source. **Suite run after the round-1 fixes: 2117/0**, ContentCheck **271/0** (medulloblastoma **5.6**, ependymoma 5.4). One failure on the way there was my own new assertion rather than the content: it asked `ContentBlocks.Compose(...).Sources` for a PAGE source, and that collection is documented as "the sources contributed by the **blocks** that were pulled in" and excludes the page's own front matter. Rewritten to read the front matter, with the API fact recorded in the test, because a guard reading the wrong collection can pass for the wrong reason just as easily as it can fail. **HARNESS RUN 1: all 155 mutations caught on LF AND CRLF**, zero survivors, clean restore, no marker left, `git status` back to the 11 intended files. Pre-flight before it (worth keeping as a step): every one of the 51 distinct (class, test) pairs was verified to exist, and every content guard on the new page and the new block was confirmed to have at least one mutation aimed at it, because `dryrun.py` checks anchors and duplicate names but NOT that a named test exists, and `break-tests.py` only discovers a typo'd name as "filter matched NO test" hundreds of invocations in. **Note on reading harness logs:** the run was launched piped through `tail -180`, which kept only the last 180 of ~313 lines, so the captured log shows 23 LF lines against 155 CRLF ones and looks like a half-run. It is not: the summary line prints only when the failure list is empty, and the script exits 1 otherwise. Do not pipe the harness through a small `tail`. **Handproof run 1: 8 of 9 correct, and the one that did not fail was the case I had just added.** `ThePageIsServedAtItsExistingUrl` still PASSED with the page's front-matter `slug:` changed, because **the route is not the slug**: `ContentStore.GetPage` maps the url straight onto a file path (`Path.Combine(Root, urlPath + ".md")`) and `ContentFrontMatter.Slug` is read by nothing in the routing at all. So that was a **WEAK MUTATION, not a weak guard** — the distinction `probe.py` exists to tell apart, and reading it the other way would have meant "fixing" a healthy test. The real break has to stop the page PARSING: `ContentStore.Parse` throws for a page "missing a title", so the case now drops the title line. (The guard's other half, WI-533's >20,000 character body, has no small unique anchor and is deliberately not broken separately.) Tree verified restored after the run: craniotomy.md absent from the diff, no marker file, the 11 intended files only. **Handproof run 2: 9/9, all render guards proved by hand** (every case FAILED when its page was broken), tree restored, craniotomy.md absent from the diff. **Rendered read 2 done, NO findings** (`rendered-medulloblastoma.txt`, 33,101 chars): every round-1 fix confirmed as SERVED — the grade paragraph now lands on WHO's reason rather than a population claim, group 3 and group 4 read as current names, the coin analogy is gone, the posterior fossa lead-in carries no comparative, "adult patients" is served, and the support-list mention of posterior fossa syndrome is PLAIN TEXT, which is the proof the tooltip now fires exactly once. A useful cross-check: `/tumors/ependymoma` rendered byte-identical to read 1 (34,417 chars both times), which is what reverting the cohort citation out of the block should do. **Privacy/secrets scan done (the repo is PUBLIC) and resolved:** no secrets file touched, no absolute local paths, and **no personal name in any new content or test file**. Three "secret" hits were the literal word *secrets* in this file's own sentence "Privacy/secrets scan done", and a fourth was the `sk-` key pattern stemming into "ri**sk-s**tratification" inside a source title — my own scan's substring holes, the same shape §12.8 keeps recording for ban lists. **One real finding, fixed:** the scan counted the GitHub account name as net +1 against HEAD (89 to 90), in the PR pre-flight line I had just written; it earns nothing in a public file and was removed. (The re-scan then caught it a SECOND time, because the sentence recording the removal had spelled the name out again. Narrating a redaction can re-introduce it.) **The tree is verified green AS IT STANDS:** since the last full run (2117/0, ContentCheck 271/0) the only edits have been to PROGRESS.md, which no test reads, and to the git-ignored harness script, so the content and test files are byte-identical to that run. **`/review` round 2 RUNNING** (briefed to VERIFY the eleven round-1 fixes against source rather than re-review from scratch, because WI-536's round 2 found one of round 1's own replacement attributions was itself wrong (§12.14); asked specifically to judge whether the residual-tumor rewrite is now under-informative, whether the reworded WHO sentence still ranks the other groups by implication more quietly, and whether banning the substring "do well" would fail a correct future sentence). **`/review` round 2: 1 BLOCKER, 5 should-fix, 5 nits, and it verified 9 of the 11 round-1 fixes as correct against source.** **BLOCKER: the medulloblastoma bullet this item added to `/treatments/chemotherapy` traced to NOTHING in that page's own front matter** — its ten sources are all adult or drug-specific, so the one claim WI-537 added was the one claim a reader could not follow (§12.2 item 2). It is the same defect the item had just fixed one layer over in the block, and the check was run on the block and not on the sibling page the same item edited. Two sources added (St. Jude for "A combination of chemotherapy medicines is used to treat medulloblastoma" and the under-3 deferral; PMC12003599 for the three drugs), and the pinning test now asserts that page's front matter carries them, because the old test pinned the bullet's PROSE and asserted nothing about where it traces. **I CORRECTED THE REVIEWER on the drug sourcing, and it mattered:** round 2 said the bare trio is verbatim ONLY in an adult paper and must not be cited for a sentence about a child. A normalised sweep found it in a PEDIATRIC context in PMC12003599 ("Common regimens include cisplatin, vincristine, and cyclophosphamide, as seen in trials like SIOP PNET 4 and COG protocols") and again in PMC10743079. **Third time this item that a review quote failed in one direction or the other** — the caution was right, the "only" was false, and following it would have cost the better citation. **Should-fix, several resolved DIFFERENTLY from what was proposed:** (1) round 1's grade-section replacement ("because that kind responds to today's treatment") was the same ungated group-level outcome claim with the adverb removed, and it slipped past the "responds well/best/better" bans round 1 had just extended — **§12.14 biting twice in one item**. Rather than the reviewer's wording I rested the warning on the grade itself: "Every medulloblastoma is grade 4 whatever its kind, so the number cannot tell you how one child will do", which names no group and makes no outcome claim, with a test pinning it and banning the responds-family on the page. (2) **"do well" MOVED from `Characterisations` to `RejectedCharacterisations`** rather than narrowed: as a bare substring it fails "ask what support helps your child do well at school", which is correct on this page's own everyday-life section and on WI-538's hub next, and that is exactly why "went well" and "back to normal" are already rejected. (3) Factoring had **dropped WI-536's "usually"** from "usually starts in hospital", leaving the block contradicting its own next paragraph, which tiers onset after you are home; restored, and the test asserts the hedged string. (4) The block's only attribution was a StatPearls chapter titled "Ependymoma" rendering on a medulloblastoma page, so the **tumour-neutral** PMC9856273 was added and StatPearls kept; round 2's alternative of moving the impairment sentence INTO the block was **rejected** because that finding is medulloblastoma survivors and a block asserts its prose on every includer. (5) **A real under-triage, inherited from WI-536:** both hubs filed new trouble walking as SAME-DAY while `[SPINAL-CORD]`, composed lower on the same page, files the same sign RIGHT AWAY for a tumour in or spread to the cord. The clause was added to **both hubs in one change** (ependymoma "in the spinal cord or has spread to the spine", medulloblastoma "has spread to the spine"), because fixing only the new page would have created the two-strengths defect the block exists to prevent. Nits taken: "more than a certain amount is left behind"; the self-blame section now ends on an action. **Suite 2117/0, ContentCheck 271/0** (medulloblastoma 5.6, ependymoma 5.4, chemotherapy 5.4 — the added citations and the added clause cost no grade). An "ependymoma.md modified on disk" warning was investigated rather than waved past: the diff carries only the two intended hunks, three directive lines and no stray mutation text, so it was byte and line-ending churn from the harness writing and restoring that file. **Rendered read 3: no new defects** — all five changed regions served, the restored hedge identical on BOTH hubs, the causes section ending on its action, no unresolved markers. **dryrun then caught a silent harness failure worth recording:** `pfs-carveout-after-tier` had become a NO-OP, because restoring "usually" rewrapped the paragraph its hard-coded literal matched. That is the "anchor text moved" case the harness docstring warns about, and it is the failure mode that makes a weak mutation read exactly like a weak guard; the mutation locates the paragraph by regex now. mutations.py **162**. Handproof deliberately NOT re-run: no render guard was touched this round, which is WI-536's stated threshold. **`/review` round 3 RUNNING**, briefed to hunt for defects the round-2 fixes introduced and to challenge three contested resolutions: the grade wording, moving "do well" to the rejected list, and my refusal to change `blocks/mechanism.md`'s "feeling sick" inside this item. **`/review` round 3: NO BLOCKERS**, 5 should-fix, 3 nits, every round-2 fix re-verified against source. **It also settled the sourcing dispute in my favour:** the drug trio is verbatim in at least four PEDIATRIC files, including PMC12003599's own table row for the under-3 radiation-sparing context, so round 2's "verbatim only in an adult paper" was false and following it would have cost the better citation. **The sharpest finding was a word collision nothing could have caught automatically:** the grade section said grade 4 "describes the tumor's **kind**", while "kind" means MOLECULAR GROUP everywhere else on the page ("the kind changes the treatment plan", "which kind the tumor is decides who gets offered testing", "the kind matters a great deal"). Read that way it claimed the grade tells you the group, which the section denies six lines later, **and both sentences were pinned by tests, so the collision was enforced** — in the section a frightened parent reads first. Now "describes this type of tumor as a whole". **The third pinned-claim defect in three rounds:** the chemotherapy bullet said "**most often** cisplatin, vincristine and cyclophosphamide", a frequency ranking of regimens no cited source makes (they say "Common regimens include", "typically", "include"), and the guard pinned it. The progression is worth recording as one lesson: round 1 a test pinning an UNSOURCED sentence, round 2 one pinning an UNATTRIBUTED claim, round 3 one pinning an OVERSTATED claim. Both pages now say "usually includes", at one strength, and both name **lomustine**, which is in the North American average-risk backbone (StatPearls; the Canadian standards' "vincristine, CCNU, cisplatin and cyclophosphamide") and was on neither page while `/treatments/chemotherapy` has a whole section about it. **Three attribution assertions passed on a COMMENT:** `Assert.Contains(url, front)` stays green if the `- url:` entry is deleted while the URL survives in a nearby comment, which is exactly the shape this block's own front matter contains for the page-local cohort study. They count `- url:` ENTRIES now, and the composed source list is asserted too, because nothing had checked that the tumour-neutral source actually REACHES either hub's rendered list, which was the entire reason for adding it. **The outcome-claim guard banned vocabulary rather than the property:** the gate's own "Treatment works well enough for the WNT kind" is the same group-level claim in words the "responds well|best|better|to today" regex cannot see, so an edit moving it OUT of the gate would have gone green. A POSITION guard was added instead (§12.14 again: banning the phrasing is not fixing the claim). Nits taken: ependymoma's four-"or" tier softened with a comma rather than split, because splitting would break the same-day/shunt adjacency the shunt guard requires; the "do well" justification corrected, because it quoted a sentence that is not on the page (§12.14 applied to a REASON, which is WI-536's "a recorded justification can be false of the page it cites"); and the embryonal gloss softened to ACS's own framing, since the fetal-remnant wording lives in a source this page bars and in one it does not cite. **On the mechanism.md deferral the reviewer improved my plan:** `WI-564 Corpus-wide British idiom sweep` ALREADY names this word and this file, so no duplicate item was raised. WI-564 instead gained (a) the composed-page guard switch, since `feeling sick` is in no shared list and every per-page idiom guard reads the RAW page, so nothing will ever turn it red, and (b) a priority note distinguishing it from WI-563's blocker, because here the phrase sits beside the unambiguous US term in the same coordination. Recorded there that four deferrals is where a deferral has become a decision. mutations.py **166, dryrun 0 problems**. Handproof deliberately NOT re-run: all nine anchors checked against this round's edits, none touched, and no render test changed. **CODE-COMPLETE 2026-09-15.** Final: **suite 2117/0** (2065 before this item, so +52 tests), **ContentCheck 271/0** (medulloblastoma **5.6**, ependymoma 5.4, chemotherapy 5.4), **harness run 2: all 166 mutations caught on LF AND CRLF**, zero survivors, clean restore, no marker, tree back to the 12 intended files. A FULL re-run was chosen over WI-536's filtered approach because rounds 2 and 3 changed prose or assertions behind roughly a dozen guards, and filtering risks missing one and shipping a guard nothing proves. **Handproof 9/9** (not re-run after round 3: all nine anchors checked against those edits, none touched, no render test changed — WI-536's stated threshold). **Four rendered reads.** **Three `/review` rounds (2 blockers, then 1, then none).** **Rendered read 4: no findings**, all four changed regions served, including the lomustine addition picking up its existing glossary tooltip. **Privacy re-scan clean** on the full diff (2535 added lines): no key-shaped strings, no absolute local paths, no credentials file in the diff, and net **+0** personal names in every changed file — the earlier alarm on `docs/backlog.md` was my own scan counting the WHOLE FILE instead of added lines, the same substring-versus-property mistake this item kept finding in guards. **The scan also flagged itself, twice**, which is why the wording here avoids the literal tokens: a grep for a token will match the sentence reporting that the token is absent, and a single-line table row shows as wholly "added" the moment any part of it is edited, so pre-existing prose inside it looks new. Net-new-versus-HEAD per file is the only comparison that means anything. Docs written: **§12.8 WI-537 entry** (fifteen lessons, headed by the three-rounds-three-pinned-claims through-line) and the **backlog done-entry**; WI-564 gained the composed-page idiom-guard switch and a priority note rather than a duplicate item being raised. **NEXT: final suite + ContentCheck on the exact content to be committed, then commit → PR into develop → release PR into main → deploy smoke check.** Pre-flight for the PR chain already done and clean: `gh` authenticated, remote is `badsonstudios/BrainHarbor`, **`develop` has NOT moved** (HEAD level with `origin/develop`, nothing committed on this branch yet), `origin/main` at `c13014c` (the WI-536 release merge). **Findings that will shape the page:** the CSF staging rule is "14 days or more after surgery" (blood contamination; pre-op LP contraindicated) with the Canadian standard saying "10-14 days" in its risk definition — a real internal inconsistency to print; radiation is normally withheld under age 3 (St. Jude; CRUK makes every under-3 high risk) and the threshold "is evolving"; **WHO itself warns that calling WNT medulloblastoma grade 4 "potentially risks giving a false sense of prognosis"**, which is the anchor for the grade section (and GliomaPageTests already cites WHO's "a grade 4 medulloblastoma does not mean what a grade 4 glioblastoma means"); histology (classic, D/N, MBEN, LC/A) and the four molecular groups are TWO AXES combined in a layered diagnosis; groups 3 and 4 need methylation profiling; posterior fossa syndrome is ~25% after this surgery, onset 1-2 days and always within the first week; adults are a different disease (SHH ~60%); WNT de-escalation trials are unpublished. **Traps:** NCI-CONNECT prints "grade IV"; "PNET" is still live in an adult guideline and GeneReviews; CRUK calls the molecular groups "subtypes" and misnumbers them; PNET3/4/5 are trial names, not diagnoses. **WI-536 is LIVE** — PR #139 into `develop` (build-test pass 3m46s), release PR #140 into `main`, and the merge commit's `main` run went **build-test success → deploy success** first time (`c13014c`). **Live smoke 8/8:** `/`, `/research`, `/get-help-now`, `/glossary`, `/tumors/ependymoma`, `/tumors/diffuse-midline-glioma`, `/tumors/glioma`, `/tumors/meningioma` all 200 with full bodies. Content probes on the live pages: the block's "or has spread to the spine" and "pain in the back or neck" (on BOTH hubs), the ambulance carve-out, "PF-EPN-A", "Grade 1 is the slowest-growing kind", the baby's irritability line, `def-posterior-fossa-syndrome`; no unresolved fence or directive; and `/tumors/meningioma` does NOT carry the block's tier, so the enforced split holds in production. The WI-536 pickup history follows. **WI-536 (Ependymoma, deepened) — started 2026-09-15**, branch `feature/wi-536-ependymoma` (from develop at 636c733). Sources being fetched by agent into `.claude/work_files/wi536-sources/` (NOTES.md, script-checked quotes). **§12.10 DECISION (spinal-cord rule):** NOT a conditional in `blocks/escalation.md` — MeningiomaPageTests and BrainMetastasesPageTests deliberately pin that block spinal-free, and brain-metastases' line is a different conditional ("if you have cancer"). Instead a SCOPED shared block `blocks/spinal-cord.md` ([SPINAL-CORD]) carrying DMG's right-away wording + CRUK source, included after [ESCALATION] by DMG and ependymoma (DMG's page-local copy removed, its tests moved to `SpinalCordBlockTests`). **Known split handed to WI-543:** meningioma, brain-metastases and spinal-cord-tumor say "seen quickly, not to wait for the next appointment" — a weaker strength than the block's "right away, at any hour". **Progress (2026-09-15):** sources DONE (51 files, 241 quotes script-checked, NOTES.md). Block + DMG refactor + `SpinalCordBlockTests` DONE. `/tumors/ependymoma` FULL DRAFT in the content tree (grade 5.3) + `EpendymomaPageTests.cs` (content + render) + glossary `posterior-fossa`, `posterior-fossa-syndrome` (suppressed on the page where defined inline). `/tumors/glioma` "Grade 2 and above are different" scoped "for most gliomas" with an ependymoma exception pointer (it contradicted ependymoma). Curability in the grade section at ACS strength ("sometimes ... when all of it can be taken out"); ABTA/2025-review "cured"/"usually curative" are SUBEPENDYMOMA only. Suite run 1: 2057/4, all four this item's (en dashes in front-matter comments, glossary "slowly get better" ranks a finding, an 8-gram shared with meningioma's NF2 line) — fixed; suite run 2 running. Harness scripts in `.claude/work_files/wi536/`: mutations.py (92, dryrun 0 problems, every test name verified), handproof.py (7 render cases), render-read.py (ependymoma, DMG, glioma). **Update, later 2026-09-15:** suite run 3 **2062/0** (ContentCheck 271/0, ependymoma 5.3, glioma 5.6). **Rendered read 1** (`rendered-read-1.md`): the glioma.md exception inserted mid-paragraph made "They are malignant ... not curable" read as ependymoma — moved to its own paragraph after the hard sentence (test pins order); a baby's pressure signs got a tier; spinal-surgery pain line aligned with [SPINAL-CORD]; glioma + supratentorial tooltips suppressed; central-canal wording fixed (St. Jude). **Rendered read 2:** nothing new. **Harness run 1:** 3 survivors on LF AND CRLF — 1 weak mutation (anchor hit the short version), 2 weak guards (case-sensitive idiom check; "not the tumor coming back" counted as an answer to a warning sign) — both guards rewritten and **harness run 2 proved them** (98 mutations total). **Handproof 9/9.** Review snapshot at `.claude/work_files/wi536/review-snapshot/`; **`/review` round 1 RUNNING** (code-reviewer agent). Lessons draft at `.claude/work_files/wi536/lessons-draft.md`. **`/review` round 1 (saved `review-1.md`): 2 BLOCKERS, 13 should-fix, nits — every quote grep-verified first (two did NOT check out: "can occur after surgery" is not in the St. Jude PFS file, and CERN never says it works "only" on ependymoma; StatPearls' "a well-known complication of posterior fossa surgery" was used instead).** B1: the block said "back pain" while the page's spinal line says "back or neck pain" (CRUK's own source: pain "anywhere in your back, spine or neck") — block now "pain in the back or neck", plus a test that reads the block and requires every sign the page names. B2: the block did not address a tumor that has SPREAD to the spine, which both including pages say can happen — now "in the spinal cord, or has spread to the spine". Should-fix taken: radiation age line rewritten (St. Jude + 2025 review say from about age 1; ACS general page says before 3; "St. Jude gives" removed); ACNS0831 carries its limits (children and young adults, tumor all or nearly all gone, "at least so far"); balance/walking/swallowing got a tier; "not the tumor coming back" (unsourced) replaced by StatPearls' complication wording; the cancer answer now leads with the grade ("Some are and some are not" went past every source for grade 2 and clashed with the block's "When cancer presses"); normalisation guard is PARAGRAPH-scoped and asserts it checked the PFS paragraph; site-only names and C11orf95-RELA added to the crosswalk; NBTS cited for the speech/vision symptoms; MemberData for the block position theory; pointer wording on BOTH hubs ("at the end of the section on when to call for help"). Rejected with reasons: dropping the "feeling sick" ban (raw-page ban is harmless) and making DMG's swallowing line carry the shunt clause (swallowing is not a shunt sign). **Suite run 4: 2063/1**, the one failure a test typo in my own new assertion, fixed. mutations.py rewritten: **126 mutations, dryrun 0 problems, every test name verified.** **Suite run 5: 2064/0** (ContentCheck 271/0, ependymoma 5.4, DMG 4.9, glioma 5.7). **Rendered read 3: no findings** (`rendered-read-3.md`) — every round-1 fix confirmed on the rendered page. **Harness run 3: all 126 mutations caught on LF AND CRLF**, clean restore, no marker. Handproof 9/9 (run before the round-1 fixes; the render tests did not change after it, but re-run it if a render guard is touched). Snapshot for round 2 at `.claude/work_files/wi536/review-snapshot-2/`. Lessons for §12.8 drafted in `lessons-draft.md`. Docs WRITTEN: backlog done-entry + §12.8 WI-536 entry (no test reads docs — checked). **`/review` round 2 (snapshot-2): 2 BLOCKERS, 6 should-fix, nits — quotes verified first (swallowing: every hit in the CITED sources is POST-OPERATIVE; the only presenting-symptom source is the barred Brain Tumour Charity; PF-EPN-A/B in StatPearls + EANO, ST-EPN-RELA in EANO).** B1: the PFS section gave "stops talking" a SAME-DAY tier while the [ESCALATION] block composed above it files sudden loss of speech as an AMBULANCE call — the page's own tier was the weaker one (WI-512's defect, in the opposite direction from WI-535's shunt blocker). Fixed with a carve-out sentence ("Suddenly not being able to speak is on the ambulance list above, and that rule still stands") and an at-home tier naming only swallowing/walking/mood. B2: "Is it cancer?" answered for grade 3 only, while the next sentence says most are grade 2 or 3 — a grade 2 reader was left to infer "not cancer" (§12.3's documented core confusion). Now: low grade "is about how fast it grows, not about how serious it is", plus surgery and years of scans. Should-fix taken: swallowing removed from the presenting list (post-op only in every cited source); the BLOCK's last sentence is now the action, not the attribution (§12.6 — it ends the when-to-call section on every including hub); the NCI-CONNECT comment no longer cites it for a grade (ACS carries that); PF-EPN-A/PF-EPN-B/ST-EPN-RELA added to the crosswalk; the block comment's claim that BOTH pages mention spread corrected (DMG does not say it in reader text); `EverySpinalSignThePageNamesIsInTheBlocksRule` now matches the block's right-away SENTENCES, not the whole block (its two assertions never met); glioma "These" → "Most of them"; test class sealed; frequency guard got a canary. **Suite run 7: 2064/0** (ContentCheck 271/0, ependymoma 5.4; run 6 had caught one em dash in a source comment I had just written — NoEmDashInCopyTests reads comments). **Harness run 4**, filtered to the eight guards changed by the round-2 fixes: **100 breaks, all caught on LF and CRLF**, clean restore. **Rendered read 4: no findings** (`rendered-read-4.md`) — the carve-out, the grade-2 answer, the walking/balance tier, the block's new landing on BOTH hubs, and the PF-EPN crosswalk bullet all confirmed as served. mutations.py now **135**, dryrun 0 problems, every test name verified. Snapshot for round 3 at `review-snapshot-3/` (md5-verified identical to the live page). **`/review` round 3: 1 BLOCKER, 5 should-fix — round-2 fixes all confirmed correct.** BLOCKER: the block's right-away tier vs "a reason to be seen quickly" on `/tumors/spinal-cord-tumor`, `/tumors/meningioma` and `/tumors/brain-metastases` — one claim, two strengths, with nothing guarding it. **Decision: the three siblings are NOT re-tiered here** (meningioma's wording was set by its own item, with its own source and `/review`, and WI-543 owns the subject and rewrites the spinal-cord stub), but the deferral is now ENFORCED: `TheStrengthSplitWithTheSiblingHubsIsPinnedUntilWi543SettlesIt` reads all three siblings, pins the divergent sentence verbatim, asserts none of them includes the block, and pins the block's own tier — so drift on either side fails. Should-fix taken: `behavior` was stranded by round 2's narrowed PFS tier (and the normalisation guard's symptom pattern had `mood` but not `behavio`); the round-2 ACS attribution comment was itself wrong (neither ACS page states the malignancy claim alone — one has the general grade framing, the other grade 3 for ependymoma); the stale "both pages say it can spread" comment in SpinalCordBlockTests corrected; a grade 1 reader (and a molecular name with no grade) now gets a scope answer; irritability added to the baby's signs (St. Jude). Nits taken: crosswalk "the same groups" → "these names"; frequency regex tightened so a widened guard could not match "in 3 weeks". **Suite run 8: 2065/0** (ContentCheck 271/0, ependymoma 5.4) — the new cross-page split test included. mutations.py **141**, dryrun 0 problems, every test name verified; the sibling-drift breaks mutate `/tumors/meningioma`, so a harness run shows 13 changed files instead of 12 until it restores. Sibling sentences verified present on all three hubs, and none includes the block. Pre-flight: `gh` authenticated, remote correct, `develop` has not moved (branch is at 636c733 with no commits yet). **Harness run 5: 56 breaks, all caught on LF and CRLF, no survivors**, clean restore (meningioma.md restored; git status back to 12 files, no marker). **Rendered read 5: no findings** (`rendered-read-5.md`) — the grade-1 scope sentence, the baby's irritability, behavior in the PFS tier, the swallowing-free symptom list and the PF-EPN crosswalk all confirmed as served. Counts pass done (backlog, §12.8, commit draft: **141 mutations, five rendered reads, 2065 tests**). Snapshot for round 4 at `review-snapshot-4/` (md5-verified). One more sourcing tidy-up before launching it: "Grade 1 is the slowest-growing kind" now names its comparative in the front matter (2025 review, subependymomas' "more indolent course relative to other ependymomas"). After that edit: build + ContentCheck clean, 222 targeted tests green (incl. NoEmDashInCopyTests, which reads comments). Snapshot-4 refreshed and md5-verified. **`/review` round 4 RUNNING** (asked explicitly to judge whether the enforced-deferral test is sufficient, or whether the strength split still blocks). **Suite run 9: 2065/0** on the exact content to be committed. **Privacy/secrets scan done** (the repo is PUBLIC): secrets clean, no absolute local paths in the new files, and no ADDED line carries a personal name — PROGRESS.md's 4 pre-existing occurrences are unchanged from HEAD (one I had just written was removed). **`/review` round 4: NO BLOCKERS** — it re-verified all six round-3 fixes against the source files (including the two weakest-looking: "Grade 1 is the slowest-growing kind" traces to the 2025 review's "more indolent course relative to other ependymomas" plus ACS's "Lower grade (grade 1 or 2) tumors tend to grow more slowly", and irritability to St. Jude's "Irritability or confusion"). 3 should-fix, all taken: (1) the deferral test's containment check read DIRECT includes only, and blocks nest five deep — it asserts on the COMPOSED page now, and a new mutation adds `[SPINAL-CORD]` to `caregiver.md` to prove the nested path fails; (2) the map of the split was overstated — `/tumors/brain-metastases` already opens with "that is its own emergency" and only its second sentence is weaker (corrected in the test comment, the WI-536 entry and the WI-543 item, so WI-543 does not re-tier a page that is close to right); (3) two quotes the page carried were not recorded in front matter (irritability; the PFS change list behind "mood or behavior"). Nit taken: "that is not a missing answer" → "that is the whole answer, not a gap in it" (reader's frame, not the page's reasoning). Nits left with reasons: the frequency regex's `in (four|three|five)` is scoped to the PFS subsection; the block reading as a postscript after the escalation sign-off is recorded for WI-543. mutations.py **142**, dryrun 0 problems, every test name verified. **CODE-COMPLETE 2026-09-15.** Final: **suite run 10 2065/0**, ContentCheck **271/0** (ependymoma **5.4**), **harness run 6: 20/20 on LF and CRLF** for the two guards round 4 touched (142 mutations total, all caught across runs 3-6), nine render guards handproofed, **six rendered reads**, **four `/review` rounds (2 blockers, then 2, then 1, then none)**. Privacy/secrets scan clean. Next: commit → PR into develop → release PR into main → deploy smoke check. **WI-535 is LIVE** — PR #137 into develop (CI 2019/0, ContentCheck 267/0), release PR #138 into main, `main` run build-test success → deploy success first time, live smoke 8/8 routes 200 plus content probes (see the 2026-09-15 log). **WI-439 is also LIVE** (#135/#136). The WI-535 pickup history follows. **WI-535 — was CODE-COMPLETE 2026-09-15, shipped.** Final: suite **2019/0**, ContentCheck 267/0 (DMG 4.9, DIPG 5.0), **harness run 5: all 128 mutations caught on LF and CRLF**, handproof 10/10, five rendered reads, `/review` ×3 (3 blockers → 1 → none; round 3's should-fix and nits applied in round 5). History of the item follows. **WI-535 (DMG + DIPG, deepened) — started 2026-09-15**, branch `feature/wi-535-dmg-dipg` (from develop at a57cc17). First drafts of BOTH hubs copied into the content tree from `.claude/work_files/wi535-draft/`; `/tumors/high-grade-glioma`'s "still short" note taken down (DMG was its last thin destination). **First pass GREEN after fix round 1** (suite 1959/0, ContentCheck 267/0, DMG 4.9, DIPG 5.0): round 1 fixed an en dash in a source title and three owned-phrase restatements (steroids, meningioma, shunts guards), dropped the unsourced "like roots" and "palliative care runs alongside treatment", and cited StatPearls NBK542184 for the thalamus. Glossary entries added: `pons`, `thalamus`, `palliative-care`. Door on `/treatments/targeted-therapy` → DMG `#dordaviprone`. Next: page tests (DMG drafted), H3 K27 tooltip suppression on DMG, harness (scripts copied to `.claude/work_files/wi535/`, NOT yet repointed), rendered reads ×2, `/review`. **Design:** DMG is the umbrella (name, grade, chemo note, dordaviprone, trials, a page-local SPINAL CORD right-away rule under [ESCALATION] sourced to CRUK); DIPG owns the pons (MRI-only diagnosis and the biopsy choice, radiation 6–7 weeks or ~3, the better stretch, sedation, palliative care early, the FDA pons exclusion) and routes to DMG `#chemotherapy` / `#dordaviprone`. Both include [MECHANISM] AND [ESCALATION]. **WI-439 is LIVE** (#135 → #136, main build-test + deploy green first time, smoke 9/9). Suite run 3 (2005 tests): 4 failures, all in the new tests or the page (first-sentence helper misuse ×2, a wrapped-comment match, and an end-of-life phrase OUTSIDE the DIPG outlook gate in the support list — removed on both pages). Harness: `mutations.py` written (63), dryrun found 1 no-op (line-wrapped anchor), fixed; `handproof.py` rewritten for 10 render cases. **Suite run 4: 2005/2005.** **Harness run 1: 4 survivors = 2 weak guards, on LF AND CRLF** — the Roman-numeral regex was case-sensitive ("**Grade IV.**" passed), and the gate's median guard was a bare `\bmedian\b` satisfied by the NEXT paragraph after the definition was deleted. Both rewritten in both test files (IgnoreCase; assert the definition sentence), plus `dipg-gate-no-median` and `dmg-spinal-drops-worsening` added — **they need their own harness run.** Handproof: 8/10 red as required so far. **Rendered read 1 done** (`.claude/work_files/wi535/rendered-read-1.md`): spinal rule under-triaged steady worsening; four unsourced phrases; three tooltip echoes; `fix-round2.py` written, NOT yet applied (the harness chain restores originals). **Harness run 2:** both rewritten guards caught; one new survivor (`dmg-spinal-drops-worsening`) → asserted by name. **Rendered read 2** (`rendered-read-2.md`): a wish-organization line leaking outlook outside the DIPG gate, a dangling "Those conversations", "usually by a needle biopsy", "cannot remove it" vs "cannot safely remove". **`/review` round 1: 3 BLOCKERS, 19 should-fix** (saved `.claude/work_files/wi535/review-1.md`), quotes grep-verified before acting: B1 tiredness "comes from the treatment" while Together lists "Fatigue or drowsiness" as a late-stage DIPG symptom; B2 swallowing given no tier ("aspiration pneumonia"); B3 the wish line (also found by read 2). Should-fix highlights: CRUK is about METASTATIC compression (tier kept, reason re-attributed, back pain and trouble walking added); trials-first needs "unless radiation is urgently needed"; FDA efficacy population was several studies, mostly adults, and also excluded spinal tumors; "a larger trial" unsourced (also on targeted-therapy); DIPG right tail needs St. Jude's "misdiagnosis" qualifier; re-irradiation reason printed and the radiation-therapy pointer (a year's gap) dropped; "put to sleep" → anesthesia. Two nits rejected with evidence (en dash title is banned by NoEmDashInCopyTests; "commonest" is corpus style in 13 pages). **Both pages REWRITTEN in full** with all of it; `fix-round3.py` superseded, not run. **Both test files rewritten** (paragraph-scoped spinal and better-stretch tests; new tiredness, swallowing, trials-caveat, grade-scope and anesthesia tests; months/years figure regex; IgnoreCase gated vocabulary; cure-family count outside the DIPG gate; word-bounded location check; dordaviprone sibling check scoped to its bullet). `/treatments/targeted-therapy` "a larger trial" → "a trial" (its test pins "That trial has not reported", kept). **Suite 7: 2012/2012, ContentCheck 267/0 (DMG 4.9, DIPG 5.0).** `mutations.py` rewritten: 92 mutations, dryrun 0 problems, every test name verified against the files. **Rendered read 3** (`rendered-read-3.md`): every rewrite region confirmed on the rendered page, nothing new. **Harness run 3: all 92 mutations caught on LF and CRLF** (first clean run; three guards beaten over runs 1–2, each rewritten and re-proved). Docs written: `docs/content-pipeline.md` §12.8 WI-535 entry and the backlog done-entry. **`/review` round 2 (saved `review-2.md`): 1 BLOCKER, 11 should-fix**, every quote grep-verified. Blocker: the new same-day lines (sleepiness, symptoms coming back) said nothing for a reader with a SHUNT, while the escalation block on the same page makes the whole same-day tier right-away for them. Should-fix: DIPG never said "accelerated"; "do not usually cure it" inside the gate was weaker than St. Jude and than `/tumors/glioma` (outside the gate) — curability now stated in both grade sections at the sibling hubs' strength with their shared landing sentence; right-tail qualifier carried in both halves; adult tail hedged; surgery-in-the-pons strength matched; "plan what matters" euphemism replaced by an action; re-irradiation attributed to what the review says; normalisation, trials-caveat, prognosis ("a year", death words) and gate guards tightened (gate now on the COMPOSED page). Rejections of the en dash and "commonest" upheld. **Round 4 written** (pages via `.claude/work_files/wi535-draft/r4-*.md`, copied in), both test files rewritten, `mutations.py` rewritten with the review's five follow-up breaks. **Suite 8: 2017/2019** — both failures were guard bugs, not page defects: `SentencesOf` merged a markdown HEADING into the first sentence under it ("Clinical trials, and why to ask early" + "Some trials start..." read as an uncaveated ask; "what is normal afterwards?" + "medicine to sleep" read as normalisation). Sentence-scanning guards now strip heading lines first. Dryrun 121/0 (one line-start anchor fixed). **Suite 9: 2019/2019. Rendered read 4 done. Harness run 4: all 121 caught on LF and CRLF.** **`/review` round 3 (saved `review-3.md`): NO blockers**; shunt fix confirmed against the block and `/treatments/shunts`; 3 small should-fix (DIPG missing "Until they report, nobody can tell you that it does."; DMG right tail "Some" vs DIPG "A few"; DMG curability source notes) + wording nits + four test tightenings (shunt clause must point to right away; allowed sentences exactly once; normaliser "usually comes from"). All in `fix-round5.py` (pages, tests, mutations; anchor-checked). Running: round 5 → dryrun → suite 10 → rendered read 5 → harness run 5. Suite 1959/0 three runs in a row, 10× the Kestrel classes green, 5 mutations caught on LF and CRLF, `/review` 0 blockers / 3 should-fix all taken. **WI-535 scouting is well along** (see its note below; a first DMG draft is at `.claude/work_files/wi535-draft/`, NOT in the content tree). Weighed and taken before WI-535: three firings today, and on `main` it silently skips the deploy. **Root cause found in the ASP.NET Core source:** both hosts in `KestrelWebApplicationFactory.CreateHost` come from ONE `DeferredHostBuilder`, whose `DeferredHost.StartAsync` does not start anything — it awaits a `TaskCompletionSource` SHARED by every host that builder builds. `_kestrelHost.Start()` completes it, so `testHost.Start()` returns instantly whether or not the TestServer has started; each app entry point runs DbUp (advisory lock) + the admin seeder on its own thread first, so when the test host loses that race `CreateClient()` hits "The server has not been started". Fix: start the hosts one after another, and wait for each host's OWN `ApplicationStarted`. Proof: a start-delay knob that fails the old code every time. WI-534 (`/treatments/shunts`) is LIVE — PR #133/#134. The historical WI-534 pickup notes follow. | 
...
| **Next up** | **WI-538 — Pediatric brain tumor, deepened** — the audience is a parent, not the patient; coordinate with WI-453 on whether children get a section or a sister site. **Carry forward from WI-537:** (1) this hub is the likely THIRD includer of `[POSTERIOR-FOSSA-SYNDROME]`, so add its slug to `PosteriorFossaSyndromeBlockTests.IncludingHubs` — that list is asserted exactly, and a new includer fails until it is registered — and read the block's tiers against the tumor before including (§12.10). The same applies to `[SPINAL-CORD]` if the hub covers tumors that reach the cord. (2) A glossary entry for **embryonal tumor** becomes worth adding at its second use, which this hub may be; WI-537 deferred it because the word was on one page that defines it inline. (3) WI-537's `/review` round 3 noted the ungated-outcome-claim guard is PAGE-LOCAL: promote it to `CuratedPage` when this hub lands, because it has the same temptation and the same audience. (4) Every same-day line a pediatric hub writes itself still owes the shunt clause, and the walking/balance tier owes the cord clause if the tumor can reach the spine. The historical WI-537 pickup notes follow. **WI-537 — Medulloblastoma, deepened** (Wave 4; depends on WI-513, done; the backlog entry asks for craniospinal radiation and the 14-day rule before the staging lumbar puncture). **Sources being fetched** by agent into `.claude/work_files/wi537-sources/` (same method as wi536: NOTES.md with verbatim quotes, script-checked with NFKC/quote/dash normalisation). **Carry forward:** (1) `/tumors/medulloblastoma` is a ~27-line stub citing only `cancer.gov/types/brain` (barred by §12.1 for naming) and carrying `[CAREGIVER]` alone — it needs the full §12.3 order with `[MECHANISM]` + `[ESCALATION]` together, plus `[CROSSWALK]`, `[CAUSES]`, `[TUMOR-BOARD]`. (2) **`[SPINAL-CORD]` applies to this hub**: medulloblastoma spreads through the fluid to the spine, and WI-536's block now opens "in the spinal cord, **or has spread to the spine**". Include it after `[ESCALATION]` **and add the slug to `SpinalCordBlockTests.IncludingHubs`** — that list is asserted exactly, so a new includer fails until it is registered. (3) **Posterior fossa syndrome is this tumor's classic association.** The glossary entries exist (`posterior-fossa`, `posterior-fossa-syndrome`), and the ependymoma page is the pattern: sudden loss of speech stays on the shared block's AMBULANCE tier via an explicit carve-out, and the page's own at-home tier names only the other signs. (4) Every same-day line the hub writes itself must say "right away if there is a shunt". (5) No prognosis figures; curability in the grade section at source strength — check CNS5 first (medulloblastoma is grade 4 for every type, and the four molecular groups are a second axis, not grades). (6) Harness: copy `break-tests.py`, `dryrun.py`, `probe.py`, `handproof.py`, `render-read.py` from `.claude/work_files/wi536/`, repoint `sys.path`/`OUT`/`PAGES` and the handproof PAGE/CLASS names, and grep every copied script for "wi536" before the first run. (7) **Do not "fix" the strength split** on `/tumors/meningioma`, `/tumors/brain-metastases` or `/tumors/spinal-cord-tumor` — it is pinned by `SpinalCordBlockTests.TheStrengthSplitWithTheSiblingHubsIsPinnedUntilWi543SettlesIt` and belongs to WI-543. |
