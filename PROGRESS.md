...
| **Next up** | **WI-542 — CNS lymphoma, deepened.** Biopsy, **not** resection, and the caveat that steroids given before biopsy can obscure the diagnosis — **verify that against live sources before publishing; it is the claim the item turns on.** Depends on: WI-519. **Carry forward for `/pm`, none of it introduced by WI-541:** the flattened-HTML render-guard defect fixed in WI-541 is still copied verbatim into `CraniopharyngiomaPageTests.cs:1391` and `PituitaryTumorPageTests.cs:1707`, both SHIPPED pages; a corpus-wide British-idiom sweep ("being sick" ×12 files, "straight away", "come round"); a reciprocal door from `/tumors/pediatric-brain-tumor` to `/tumors/craniopharyngioma`; **WI-566** (ReaderText silently eats three characters on six live guards across four pages); and six stale local feature branches that `git branch --merged` cannot see because this repo squash-merges. |
