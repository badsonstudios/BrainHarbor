...
| **In progress** | **WI-544 (ATRT) - STARTED 2026-09-19**, branch `feature/wi-544-atrt` (from `develop`). WI-543 is SHIPPED AND LIVE, do not reopen it. **A NEW page, not a deepened stub, which breaks a six-item streak**: there was no `atrt.md`, so no barred `cancer.gov/types/brain` citation to remove and no prior wording to retire. The corpus mentioned this tumor NOWHERE (the only `rhabdoid` hit was MeningiomaPageTests' ruling about *rhabdoid meningioma*, a DIFFERENT entity, not conflated). Reader is a PARENT of a child usually under 3 (median age at diagnosis 1 year). **BASELINES MEASURED BEFORE A WORD WAS DRAFTED:** suite **2,313/2,313**, ContentCheck **273/0**, and all four collision probes validated against a POSITIVE CONTROL first, because a zero from a freshly retargeted tool is this project's most suspicious output. **VERIFICATION IS COMPLETE AND IT CHANGED FOUR THINGS.** Three agents returned 100+ sources; I then re-fetched and re-read **19 publications myself** (a dossier cannot support a published claim). That caught: (1) **a recorded ABSENCE that was FALSE** - the pack said no ATRT patient page names a whole-spine MRI, and St. Jude's does, so the front matter would have carried a false REASON; (2) the CNS5 grade absence is real but that grade table contains **no embryonal tumor at all**, not even medulloblastoma, so "WHO declined to grade it" is false and `/tumors/acoustic-neuroma`'s framing must NOT be reused; (3) the pack **merged two shunt rules** that the source keeps apart (emergency room belongs to suspected INFECTION; malfunction gets no venue); (4) the corroborating fontanelle source **names no venue**, so the 911 wording is single-sourced and says so. **THE CENTRAL RULING: SEVENTEEN ATRT PAGES, ZERO EMERGENCY RULES.** Four re-tested by direct question, all silent. So the page invents no tumor-level emergency rule and carries SIGN-level and DEVICE-level rules, each attributed (bulging fontanelle while calm and upright, shunt you cannot wake, suspected shunt infection, raised pressure). **BLOCKS 6 IN / 2 OUT.** `[MECHANISM]` EXCLUDED because its central sentence ("The skull is a closed box. It cannot stretch.") is FALSE for an infant and is the opposite of this page's own explanatory fact; `[TUMOR-BOARD]` EXCLUDED because the decision framework here is a TRIAL PROTOCOL, not a meeting. `atrt` registered in **SpinalCordBlockTests** and **PosteriorFossaSyndromeBlockTests** (both exact-set lists; suite total moved 2,313 to 2,315, which is how we know the registrations are live). **PROOF SO FAR:** ContentCheck **274 checks / 0 failures** at grade **5.5**; full suite **2,315/2,315 green** (real unpiped exit 0) BEFORE the new test file; restatement variants 1 AND 2 both **0 files / 0 shingles** after FIVE rewrite passes, **three of which CREATED new collisions** (a fix is where the next defect comes from, three times in one item). **Two corpus-wide guards caught real defects the offline scans missed:** `travelled` (British form) and a Roman-numeral sentence that missed the allowance by one word order. The British one is a lesson worth keeping: my own scan reported the body clean because it ran in the same tool block as the edit that introduced the word, so it read a STALE FILE. **STATE: REVIEW, HARNESS AND HANDPROOF ARE ALL DONE. ONLY THE PRIVACY SCAN, COMMIT AND THE TWO PRs REMAIN.** Suite **2,345/2,345** green (baseline was 2,313), ContentCheck **274 checks / 0 failures** at grade **5.5**, restatement variants 1 AND 2 both **0/0**. **ELEVEN `/review` rounds**: blockers in rounds 1-10, and round 11 found NONE, which is the gate the order requires before any mutating tool runs. In rounds 2, 4 and several others the blocker was CREATED by the previous round's fix. **Fourteen rendered reads**; the first found THREE defects that existed only in the COMPOSED page and were invisible in the source (two near-identical `###` headings 24 lines apart, a duplicated reassurance, and a claim that nothing here is about driving while the composed caregiver block discusses driving). All 17 sections end on an action or a reassurance (§12.6). **BREAK HARNESS RUN: 93 mutations, 186/186 ok — "all 93 breaks fail correctly, on LF and on CRLF", exit 0**, covering all **23** content guards, with a PERMANENT coverage assertion that reads the real guard list out of the test file. **It was NOT first-run: three survivors.** Two were weak MUTATIONS (a property recorded in two places cannot be broken by a single-point edit), and the third was a genuinely VACUOUS GUARD — `SentencesOf` splits on `(?<=[.!?])\s+` and `ReaderText` does not strip `**`, so the shunt-INFECTION venue assertion had been passing on the venue belonging to the bullet ABOVE it for the whole item. Invisible to review, to the suite and to the dry run; only mutation could see it. **HANDPROOF RUN: 8 cases across all 7 render guards, every one red when broken, restored, exit 0.** **THE PRE-MERGE SMOKE FALSIFICATION IS DONE AND IT FAILED AS REQUIRED**, exit 1, with `controls ok (2)` FIRST and `/tumors/atrt -> HTTP 404` as the only failure. That is the separation the probe exists for: exit 3 would have meant a broken probe, exit 1 means the deploy has not landed. Cleaner than WI-543's, which had to reason about a stub returning 200. Pre-deploy baseline captured against live production. **Six candidate smoke negatives were rejected by COUNTING rather than reasoning**, each of which would have failed a healthy deploy: "cerebellar mutism" (renders via a glossary alias, and the one I expected to be absent), "tumour" x3 in block-contributed source titles, "Gy"/"gy" x8 inside "surgery", "911" x3, "emergency room" x4, and "Cancer Research UK" x3. **Both new tools were falsified rather than trusted:** the offline normalisation replica on 9 cases including both negated-tier forms, and the coverage assertion on all three of its branches. **NEXT, in order:** §12.8 lessons, backlog checkbox and this row are DONE. Remaining: privacy scan against the FINAL diff, commit, PR into `develop`, release PR into `main`, and the POST-deploy smoke run (the PRE-merge falsification is already done and failed as required). **FOR /pm (new):** `/tumors/pediatric-brain-tumor` is a SHIPPED page that composes `[MECHANISM]` ("the skull ... cannot stretch") and then says a baby's skull "can stretch" about thirty lines later, so both reach one reader in one composed page; Nationwide Children's ATRT page prints **SMARCB4**, a gene that does not exist, so it must never be cited for a gene name; Pediatric Brain Tumor Foundation and Alex's Lemonade returned 403 on seven URLs and are deliberately NOT linked. |
| **WI-543 (previous)** | **WI-543 is SHIPPED AND LIVE** (PR [#153](https://github.com/badsonstudios/BrainHarbor/pull/153) → `develop`, release PR [#154](https://github.com/badsonstudios/BrainHarbor/pull/154) → `main`, merge `7f2f6ab`, build-test success 4m4s → deploy success 1m4s, **first attempt — the fifth release in a row to deploy first time**). **WI-544 (ATRT) is next.** |
