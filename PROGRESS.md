...
| **In progress** | nothing mid-flight. (2026-08-16: **WI-412a live** — `/tumors` had shipped with no way to reach it; nav, footer, sitemap and the `/research` "What is this?" link all added and verified on prod, PR #46 → #47.) WI-401, WI-414, WI-415 all done and **released to prod** (PRs #17, #19). **Daily scheduled task registered 2026-08-13** ('BrainHarbor Pipeline', 06:00, runs as Dan, StartWhenAvailable) — the feed now updates itself, and since **WI-417** each run leaves a log behind. |
