...
- **Two kinds of failure, and the difference decides what the RUN does (WI-413).** *Unusable output* (garbled JSON, wrong shape, failed validation) is about that item: it uploads unclassified/unsummarized for a person and the cursor advances past it, because an item that can never be handled must not stall a source forever. *Unavailable* (the CLI never answered — spawn failure, timeout, non-zero exit, or output that is not the documented envelope) is about the run: the source stops on the **first** one, its cursor is held so the next run refetches the window, and the rest of the run is skipped because the CLI is shared. Uploading during an outage is what makes items unrecoverable — the server then knows them, so no later run re-classifies them. When the two are hard to tell apart (a timeout looks like both), the pipeline **asks the CLI directly** with a trivial health prompt rather than inferring. Trial facts still refresh while the CLI is down; they need no model call.
