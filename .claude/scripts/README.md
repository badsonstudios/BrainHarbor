...
|--------|--------------|---------|
| `new-pr` | Branch (if on base), commit staged changes, push, open a PR with `gh` | `./new-pr.sh -t "WI-204: PubMed fetcher" -a` |
| `load-env` | Load `.env` vars into the **current** shell (must be sourced/dot-sourced) | `. ./load-env.ps1` |
| `get-secret` | Print a single value from `.env` (without dumping the whole file) | `./get-secret.sh NCBI_API_KEY` |

*(ClaudeMon's `bump-version`/`publish-release` were not carried over — BrainHarbor
has no installer or GitHub releases. Deploys happen via GitHub Actions from M4.)*
...
- PowerShell: `./new-pr.ps1 -Title "WI-204: PubMed fetcher" [-Body "..."] [-Base main] [-Branch feature/wi-204-pubmed] [-All]`
- bash: `./new-pr.sh -t "WI-204: PubMed fetcher" [-b "..."] [-B main] [-n feature/wi-204-pubmed] [-a]`
